...
C:/Users/lukin2dmaterials/miniconda3/envs/qcodes/Lib/site-packages/qcodes_contrib_drivers/drivers/NationalInstruments/class_file. Slashes are reversed to run
            ...
            self.z = z_init = round(float(xy_scan_z_piezo_voltage_qlineedit.text()), 3)
...
C:/Users/lukin2dmaterials/miniconda3/envs/qcodes/Lib/site-packages/qcodes_contrib_drivers/drivers/NationalInstruments/class_file. Slashes are reversed to run
            ...
            self.hor_coord = 0; self.ver_coord = 0
            self.isStopped = 0; self.scanType = 'xz'

            ############################################################################### def other variables #####################################################################
            ################### setting variales and array ####################

            # counter read time:
            self.time_acquire = float(xz_scan_read_time_qlineedit.text())/1e3                                     # note a reading time <0.05 s is likely too short
            self.samps_per_chan = int(self.time_acquire * self.samp_rate)
            ...
            self.ver_init = z_init = round(float(xz_scan_z_piezo_min_voltage_qlineedit.text()),3)

            # set desired end mirror voltages
            x_final = round(float(xz_scan_x_voltage_max_qlineedit.text()),3)
            z_final = round(float(xz_scan_z_piezo_max_voltage_qlineedit.text()),3)
            ...
            self.hor_range = x_range = x_final - x_init
            self.ver_range = z_range = z_final - z_init
            x_step = np.absolute(x_range) / grid_size_x
            z_step = np.absolute(z_range) / grid_size_z
            ...
            ################### resetting position of mirrors ####################
            scan_galvo.voltage_cdaq1mod2ao0(x_init) # this is for setting the initial x voltage
            scan_galvo.voltage_cdaq1mod2ao1(y_init) # this is for fixing the x-mirror at defined setting (voltage)
            scan_galvo.voltage_cdaq1mod2ao2(z_init) # starting z-piezo at 0.0 V (0 microns); this variable will increase during scanning

            ########################################################### setting up NI-DAQmx tasks and channels for counting ########################################################

            with nidaqmx.Task() as ctrtask, nidaqmx.Task() as clktask: # this defines 2 NI-DAQmx tasks (one for the counter and one for the counter's clock)

                # adding dig pulse train chan
                clktask.co_channels.add_co_pulse_chan_freq(
                    counter = "cDAQ1Mod1/ctr1",
                    name_to_assign_to_channel = "",
                ...
                # cfg implict timing
                clktask.timing.cfg_implicit_timing(
                    sample_mode = AcquisitionType.CONTINUOUS,
                    samps_per_chan = 1
                    )

                # adding count egdes chan
                ctrtask.ci_channels.add_ci_count_edges_chan(
                    counter = "cDAQ1Mod1/ctr0",
                    name_to_assign_to_channel = "",
                    edge = Edge.RISING,
                    initial_count = 0,
                ...
                # clktask.start() # this starts the counter NI-DAQmx task
                # ctrtask.start() # this starts the hardware-based internal clock NI-DAQmx task
                        
                ####################################################################### x and z scanning #########################################################################
                output_value = 0
                for f in range(grid_size_z): # this loops for stacks in z
                    if self.isStopped == 1: break
                    clktask.start() # this starts the counter NI-DAQmx task
                    ctrtask.start() # this starts the hardware-based internal clock NI-DAQmx task

                    for k in range(grid_size_x): # this loops for columns (varying y voltage/setting)
                        if self.isStopped == 1: break
                        
                        allSamples = ctrtask.read(number_of_samples_per_channel = self.samps_per_chan)
                        elapsed = (self.samps_per_chan-1) / self.samp_rate
                        output_value = (allSamples[-1] - allSamples[0]) / elapsed

                        if self.isSnake == 1:
                            if f % 2 == 0: # this loop populates the created xy_scan_data_array (the if else structure is present bc of the snaking scanning pattern)
                                xz_scan_data_array[f][k] = output_value # add counter result to data array
                            else:
                                xz_scan_data_array[f][grid_size_x-1-k] = output_value # add counter result to data array  

                            if f % 2 == 0: # this loop adjusts for sweeping back and forth along each alternating row
                                if k < (grid_size_x - 1):
                                    x_next += x_step # increment drive voltage forwards
                                    scan_galvo.voltage_cdaq1mod2ao0(x_next) # step x motor
                            else:
                                if k < (grid_size_x - 1):
                                    x_next -= x_step # increment drive voltage backwards
                                    scan_galvo.voltage_cdaq1mod2ao0(x_next) # step x motor

                        else:
                            xz_scan_data_array[f][k] = output_value
                            if k < (grid_size_x - 1):
                                x_next += x_step # increment drive voltage forwards
                                scan_galvo.voltage_cdaq1mod2ao0(x_next) # step x motor
                            else:
                                x_next = x_init
                                scan_galvo.voltage_cdaq1mod2ao0(x_next)
                    ...
                    self.sc.axes.set_xticks(np.arange(0, grid_size_x + 1, grid_size_x / 2), [x_init, int((x_init + x_final) / 2), x_final])
                    self.sc.axes.set_yticks(np.arange(0, grid_size_z + 1, grid_size_z / 2), [int(z_init * 10), (int(z_init * 10) + int(z_final * 10)) / 2, int(z_final * 10)])
                    self.sc.axes.xaxis.set_tick_params(labelsize = 8)
                    self.sc.axes.yaxis.set_tick_params(labelsize = 8)
                    self.sc.axes.set_xlabel("x_mirror_driving_voltage_(V)", fontsize = 8)
                    ...
                    clktask.stop() # this stops the counter NI-DAQmx task - free-ing the reserved cDAQ card resources
                    ctrtask.stop()
                    

                    ##################### move in z ####################
                    if f < (grid_size_z - 1): # this loop
                        z_next += z_step # increment drive voltage
                        scan_galvo.voltage_cdaq1mod2ao2(z_next) # step z

                # clktask.stop() # this stops the counter NI-DAQmx task - free-ing the reserved cDAQ card resources
                # ctrtask.stop() # this stops the hardware-based internal clock NI-DAQmx task - free-ing the reserved cDAQ card resources
            ...
            self.sc.axes.set_xticks(np.arange(0, grid_size_x + 1, grid_size_x / 2), [x_init, np.round((x_init + x_final) / 2, 3), x_final])
            self.sc.axes.set_yticks(np.arange(0, grid_size_z + 1, grid_size_z / 2), [z_init * 10, np.round((z_init + z_final) * 10/2, 3), z_final * 10])
            self.xz_scan_plot_colorbar = self.sc.fig.colorbar(xz_scan_plot, ax = self.sc.axes, pad = 0.02, aspect = 15)
            ...
            self.crosshair, = self.sc.axes.plot(0,0, marker="+", markersize=5, markeredgecolor="red", markerfacecolor="red")
            self.sc.draw()
...
C:/Users/lukin2dmaterials/miniconda3/envs/qcodes/Lib/site-packages/qcodes_contrib_drivers/drivers/NationalInstruments/class_file. Slashes are reversed to run
            ...
            self.hor_coord = 0; self.ver_coord = 0
            self.isStopped = 0; self.scanType = 'yz'

            ############################################################################### def other variables #####################################################################
            ################### setting variales and array ####################

            # counter read time:
            self.time_acquire = float(yz_scan_read_time_qlineedit.text())/1e3        
            self.samps_per_chan = int(self.time_acquire * self.samp_rate)                                                                       # note a reading time <0.05 s is likely too short
            ...
            self.ver_init = z_init = round(float(yz_scan_z_piezo_min_voltage_qlineedit.text()),3)
            ...
            z_final = round(float(yz_scan_z_piezo_max_voltage_qlineedit.text()),3)

            # def the internal stepping voltages based on user-entered settings above
            self.hor_range = y_final - y_init
            self.ver_range = z_final - z_init
            y_step = np.absolute(self.hor_range) / grid_size_y
            z_step = np.absolute(self.ver_range) / grid_size_z
            ...
            ################### resetting position of mirrors ####################
            scan_galvo.voltage_cdaq1mod2ao0(x_init) # this is for fixing the x-mirror at defined setting (voltage)
            scan_galvo.voltage_cdaq1mod2ao1(y_init) # this is for setting the initial x voltage
            scan_galvo.voltage_cdaq1mod2ao2(z_init) # starting z-piezo at 0.0 V (0 microns); this variable will increase during scanning
            ...
            ####################################################################### y and z scanning #########################################################################
                output_value = 0
                for f in range(grid_size_z): # this loops for rows (z)
                    if self.isStopped == 1: break
                    clktask.start() # this starts the counter NI-DAQmx task
                    ctrtask.start() # this starts the hardware-based internal clock NI-DAQmx task

                    for k in range(grid_size_y): # this loops for columns (y)
                        if self.isStopped == 1: break
                        ...
                        if self.isSnake == 1:
                            if f % 2 == 0: # this loop populates the created xy_scan_data_array (the if else structure is present bc of the snaking scanning pattern)
                                yz_scan_data_array[f][k] = output_value # add counter result to data array
                            else:
                                yz_scan_data_array[f][grid_size_y-1-k] = output_value # add counter result to data array  

                            if f % 2 == 0: # this loop adjusts for sweeping back and forth along each alternating row
                                if k < (grid_size_y - 1):
                                    y_next += y_step # increment drive voltage forwards
                                    scan_galvo.voltage_cdaq1mod2ao1(y_next) # step x motor
                            else:
                                if k < (grid_size_y - 1):
                                    y_next -= y_step # increment drive voltage backwards
                                    scan_galvo.voltage_cdaq1mod2ao1(y_next) # step x motor

                        else:
                            yz_scan_data_array[f][k] = output_value
                            if k < (grid_size_y - 1):
                                y_next += y_step # increment drive voltage forwards
                                scan_galvo.voltage_cdaq1mod2ao1(y_next) # step x motor
                            else:
                                y_next = y_init
                                scan_galvo.voltage_cdaq1mod2ao1(y_next)
                    
                    ##################### updating plot section ####################
                    self.sc.axes.pcolormesh(yz_scan_data_array, cmap = "inferno")
                    self.sc.axes.set_xticks(np.arange(0, grid_size_y + 1, grid_size_y / 2), [y_init, (y_init + y_final) / 2, y_final])
                    self.sc.axes.set_yticks(np.arange(0, grid_size_z + 1, grid_size_z / 2), [(z_init * 10), (z_init + z_final)*10/2, int(z_final * 10)])
                    self.sc.axes.xaxis.set_tick_params(labelsize = 8)
                    self.sc.axes.yaxis.set_tick_params(labelsize = 8)
            ...
            self.sc.axes.set_xticks(np.arange(0, grid_size_y + 1, grid_size_y / 2), [y_init, round((y_init + y_final) / 2, 3), y_final])
            self.sc.axes.set_yticks(np.arange(0, grid_size_z + 1, grid_size_z / 2), [z_init * 10, round((z_init + z_final) *10/2, 3), z_final * 10])
            self.sc.axes.set_xlabel("y_mirror_driving_voltage_(V)", fontsize = 8)
            self.sc.axes.set_ylabel("objective_z-piezo_height_(microns)", fontsize = 8)
