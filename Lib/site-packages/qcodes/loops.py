...
class Loop(Metadatable):
    ...
    def __init__(self, sweep_values, delay=0, station=None,
                 progress_interval=None, sleepTimeAfterFinishing=0, ifIndividualCount=False, 
                 numOfPBLoops=1, ifMultReads=0, nread=1):
        super().__init__()
        if delay < 0:
            raise ValueError(f"delay must be > 0, not {repr(delay)}")

        self.sweep_values = sweep_values
        self.ifIndividualCount = ifIndividualCount
        self.ifMultReads = ifMultReads
        self.numOfPBLoops = numOfPBLoops
        self.nread = nread
        self.delay = delay
        ...
        self.sleepTimeAfterFinishing = sleepTimeAfterFinishing
    ...
    def each(self, *actions):
        ...
        return ActiveLoop(self.sweep_values, self.delay, *actions, sleepTimeAfterFinishing=self.sleepTimeAfterFinishing,
                          ...
                          ifIndividualCount=self.ifIndividualCount, numOfPBLoops=self.numOfPBLoops, 
                          ifMultReads=self.ifMultReads, nread=self.nread)
...
class ActiveLoop(Metadatable):
    ...
    def __init__(self, sweep_values, delay, *actions, then_actions=(),
                 ...
                 bg_final_task=None, bg_min_delay=None, sleepTimeAfterFinishing=0, ifIndividualCount=False, 
                 numOfPBLoops=1, ifMultReads=0, nread=1):
        super().__init__()
        self.sweep_values = sweep_values
        self.ifIndividualCount = ifIndividualCount
    ...
    def then(self, *actions, overwrite=False):
        ...
        loop = ActiveLoop(self.sweep_values, self.delay, *self.actions,
                          then_actions=self.then_actions, station=self.station, sleepTimeAfterFinishing=self.sleepTimeAfterFinishing,
                          ifIndividualCount=self.ifIndividualCount, numOfPBLoops=self.numOfPBLoops, 
                          nread=self.nread, ifMultReads=self.ifMultReads)
    ...
    def containers(self):
        ...
        """
        if self.ifIndividualCount:
            loop_size = (len(self.sweep_values), self.numOfPBLoops)
        elif self.ifMultReads:
            loop_size = (len(self.sweep_values), self.nread)
        else:
    ...
    def run(self, use_threads=False, quiet=False, station=None,
        ...
        try:
            ...
            if not quiet:
                ...
                time.sleep(self.sleepTimeAfterFinishing)
