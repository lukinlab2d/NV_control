...
class Pulse(object):
    ...
    def duration(self, duration):
        assert duration > 0, 'pulse duration has to be of finite duration but is {:f}'.format(duration)
...
class PulseBlaster(Instrument):
    ...
    LONG_DELAY_THRESHOLD = 2**32
    MAX_LOOPS = 2**20; MAX_LONG_DELAY = 2**20
    ...
    def __init__(self, name: str, verbose=True, **kwargs) -> None:
        try:
        ...
        super(PulseBlaster, self).__init__(name, **kwargs)
        self.estimated_runtime = None
        self.sequence_start_time = None
        self.hasRunOnce = None
        self.verbose = verbose
        if verbose:
            # print('Is there a global PB?')
            # if pb is not None:
            #     print('Yes')
            # else:
            #     print('No')
            print()
            print('PulseBlaster initialized')
    ...
    def merge_overlapping_pulses(self,pulse_collection,overlapping_pulses):
        # start_time_list = np.array([p.start_time for p in overlapping_pulses])
        # end_time_list = np.array([p.end_time for p in overlapping_pulses])
        # id_list = [p.channel_id  for p in overlapping_pulses]

        for pair in overlapping_pulses:
            p1 = pair[0]; p2 = pair[1]
            delay = p1.start_time; end = p2.end_time
            duration = end-delay

            # delay = np.min(start_time_list); duration = np.max(end_time_list) - delay
            pulse_collection.remove(p1); pulse_collection.remove(p2)
            pulse_collection += [Pulse(p1.channel_id, delay, duration=int(duration))]
    ...
    def create_physical_pulse_seq(self, pulse_collection):
        ...
        clock_T = 1.0e3 / self.settings['clock_speed']  # clock period in ns, clock speed in MHz
    ...
    def turn_on_infinite(self, channels, verbose=False):
        pb_commands = []; channel_address = 0
        
        for channel in channels: 
            channel_address += 2**channel
       
        pb_commands.append(self.PBCommand(int(channel_address), 100, command='CONTINUE', command_arg=0))
        pb_commands.append(self.PBCommand(int(channel_address), 100, command='BRANCH', command_arg=0))
        if verbose: print(pb_commands)
    ...
    def turn_on_infinite_without_init(self, channel, verbose=False):
        pb_commands = []
        pb_commands.append(self.PBCommand(int(2**channel), 100, command='CONTINUE', command_arg=0))
        pb_commands.append(self.PBCommand(int(2**channel), 100, command='BRANCH', command_arg=0))
        if verbose: print(pb_commands)
    ...
    def turn_off(self):
        pb_commands = []
        pb_commands.append(self.PBCommand(0, 100, command='CONTINUE', command_arg=0))
        pb_commands.append(self.PBCommand(0, 100, command='BRANCH', command_arg=0))
    ...
    def program_pb(self, pulse_collection, num_loops=1):
        ...
        assert len(pulse_collection) > 0, 'pulse program must have at least 1 pulses'
        assert num_loops < (1 << 20), 'cannot have more than 2^20 (approx 1 million) loop iterations'
        overlapping_pulses = self.find_overlapping_pulses(pulse_collection)
        while overlapping_pulses != []:
            # print('These are the overlapping pulses: ', overlapping_pulses)
            print('Merging overlapping pulses...')
            pulse_collection = self.merge_overlapping_pulses(pulse_collection, overlapping_pulses)
            overlapping_pulses = self.find_overlapping_pulses(pulse_collection)

        if overlapping_pulses != []:
            print('These are the overlapping pulses: ', overlapping_pulses)
        ...
        self.delayed_pulse_collection = self.create_physical_pulse_seq(pulse_collection)
        self.num_loops = num_loops
        self.estimated_runtime = self.estimate_runtime(self.delayed_pulse_collection, num_loops)
        pb_state_changes = self.generate_pb_sequence(self.delayed_pulse_collection)
        pb_commands = self.create_commands(pb_state_changes, num_loops)
        if self.verbose:
            if len(pb_commands) < 20:
                print(pb_commands)
                print('len(pb_commands) = ' + str(len(pb_commands)))
                print()
            else: 
                print('len(pb_commands) = ' + str(len(pb_commands)))
                print()

        # print('len(pb_commands)', len(pb_commands))
        ...
        for command in pb_commands:
            ...
            if command.duration < self.settings['min_pulse_dur']:
                raise RuntimeError("Detected command with duration < min_pulse_dur ns.")
            if command.command == 'LOOP' and command.command_arg > self.MAX_LOOPS:
                raise RuntimeError("Detected command with more than 2**20 loops")
            if command.command == 'LONG_DELAY' and command.command_arg > self.MAX_LONG_DELAY:
                raise RuntimeError("Detected command with more than 2**20 long delays")
    ...
    def start_pulse_seq(self):
        ...
        """
        self.estimated_runtime = self.estimate_runtime(self.delayed_pulse_collection, self.num_loops)
        self.pb.pb_start()
        assert self.pb.pb_read_status() & 0b100 == 0b100, 'pulseblaster did not begin running after start() called.'
        if(self.settings['PB_type'] == 'PCI'): #leave USB PB connection open to stop it later
            self.pb.pb_close()
        ...
        if not self.hasRunOnce == True and self.ifPrintTime == True:
            if self.estimated_runtime <= 1000:
                print('self.estimated_runtime = ' + str(self.estimated_runtime) + ' ms')
            else: 
                print('self.estimated_runtime = ' + str(self.estimated_runtime/1000) + ' s')
        if self.hasRunOnce == None:
    ...
    def stop_pulse_seq_without_closing(self):
        """
        Stops pulse sequence. Only needs to be called for USB pulseblasters. Requires that the PB-computer connection
        is still open.
...
class B00PulseBlaster(PulseBlaster):
    _PROBES = {}
    ...
    """
    def __init__(self, name: str, settings: Dict[str, any], verbose=True, ifPrintTime=True, **kwargs) -> None:
        super(B00PulseBlaster, self).__init__(name, verbose=verbose, **kwargs)
        
        channels = {}
        for key in settings:
            if isinstance(settings[key], Dict):
                if 'channel' in settings[key]:
                    channels_key = str(key) + '_channel'
                    channels[channels_key] = settings[key]['channel']
        ...
        if verbose:
            print('B00PulseBlaster initialized')
            print()
        # self.turn_off() # initialize to outputing no pulses
        self.stop_pulse_seq()
        self.ifPrintTime = ifPrintTime

    def update(self, settings):
        # call the update_parameter_list to update the parameter list
        # oh god this is confusing
        super(B00PulseBlaster, self).update(settings)
...
if __name__ == '__main__':
    ...
    clock_speed = 500 # MHz
    settings = {'clock_speed': clock_speed, 'AFG': AFGParam, 'MWswitch': MWswitchParam, 'PB_type': 'USB',
                'min_pulse_dur': int(5*1e3/clock_speed)}

    pb = B00PulseBlaster("SpinCorePB", settings=settings)

    num_loops = 1000000
    pulse_sequence = []
    pulse_sequence += [Pulse('AFG', 20e3, duration=int(10e3))] # times are in ns
    pulse_sequence += [Pulse('MWswitch', 20e3, duration=int(10e3))]
    ...
    for i in range(int(2e5)):
        pb.turn_on_infinite(channel=3, verbose=False)
        time.sleep(1e-3)
        pb.turn_off()
        time.sleep(1e-3)
    
    # pulse_sequence += [Pulse('MWswitch', 1000e3, duration=int(100e3))]
    # pulse_sequence += [Pulse('MWswitch', 1200e3, duration=int(100e3))]
    # pulse_sequences = [pulse_sequence, pulse_sequence, pulse_sequence, pulse_sequence, 
    #                    pulse_sequence, pulse_sequence, pulse_sequence]

    # num_daq_reads = 0
    # for pulse in pulse_sequences[0]:
    #     if pulse.channel_id == 'MWswitch':
    #         num_daq_reads += 1
    # signal = [0.0]
