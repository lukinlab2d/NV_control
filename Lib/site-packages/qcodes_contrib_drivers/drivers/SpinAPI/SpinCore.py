...
class B00PulseBlaster(PulseBlaster):
    ...
    """
    def __init__(self, name: str, dev_name: str, settings: Dict[str, any], **kwargs) -> None:
        super(B00PulseBlaster, self).__init__(name, **kwargs)
        channels = {}
        for key in settings:
...
if __name__ == '__main__':
    ...
    apd_readoutParam = {'delay_time': 2, 'channel':2}
    clock_speed = 500 # MHz
    settings = {'clock_speed': clock_speed, 'laser': laserParam, 'apd_readout': apd_readoutParam, 'PB_type': 'USB',
                'min_pulse_dur': int(5*1e3/clock_speed)}

    pb = B00PulseBlaster("name_two", scan_galvo_card_name, settings=settings)

    num_loops = 100
    pulse_sequence = []
    pulse_sequence += [Pulse('laser', 5e3, duration=int(10e3))]
    pulse_sequence += [Pulse('apd_readout', 20e3, duration=int(20e3))]

    pulse_sequence += [Pulse('apd_readout', 1000e3, duration=int(100e3))]
    pulse_sequence += [Pulse('apd_readout', 1200e3, duration=int(100e3))]
