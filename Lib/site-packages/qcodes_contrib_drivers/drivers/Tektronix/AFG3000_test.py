...
class AFG3000(VisaInstrument):
    ...
    def set_freq(self, freq: float, src: int):
        self.write(f"SOURce{src}:FREQuency:FIXed {freq}MHz")

    def set_high(self, highamp: float, src:int):
        self.write(f"SOURce{src}:VOLTage:LEVel:IMMediate:HIGH {highamp}V")

    def set_low(self, lowamp: float, src:int):
        self.write(f"SOURce{src}:VOLTage:LEVel:IMMediate:LOW {lowamp}V")

    def burst_mode_on(self, cycles: int, delay: float, src:int):
        self.write(f"SOURce{src}:BURSt:STATe ON")
        self.write(f"SOURce{src}:BURSt:MODE TRIGgered")
        self.write(f"SOURce{src}:BURSt:NCYCles {cycles}")
        self.write(f"SOURce{src}:BURSt:TDELay {delay}ns")

    def burst_mode_off(self, src:int):
        self.write(f"SOURce{src}:BURSt:STATe OFF")
