...
class SRS():
    def __init__(self, name='SRS_SG386', SRSnum=1, settings=None, **kwargs) -> None:
        ...
        self.SRSnum = SRSnum
        if SRSnum == 1:
            self.srs = rm.open_resource('GPIB1::27::INSTR') # temporary
            # self.srs = rm.open_resource('COM' + str(6))
            # self.srs.baud_rate = 115200
            self.srs.query('*IDN?')
        elif SRSnum == 2:
            self.srs = rm.open_resource('COM' + str(11))
            self.srs.baud_rate = 115200
            self.srs.query('*IDN?')
        elif SRSnum == 3:
    ...
    def enableIQmodulation(self):
        ...
        self.srs.write('MODL 1') #Enable modulation
        self.errorCheck()
        
        self.srs.write('TYPE 6') #Set modulation type to IQ
        self.errorCheck()
       
        self.srs.write('QFNC 5')  #Set IQ modulation function to external
    
    def enableNoiseModulation(self):
        if self.SRSnum != 3:
            self.srs.write('MODL 1') # Enable modulation
            self.srs.write('TYPE 2') # Set modulation type to phase mod
            self.srs.write('MFNC 4') # Set modulation function to noise
            self.srs.write('RATE 45 kHz') # Set noise bandwidth

        else:
            self.srs.write('LFO:FREQ 10kHz')
            self.srs.write('LFO ON')
            self.srs.write('PM:SOUR INT')
            self.srs.write('PM:STAT ON')
