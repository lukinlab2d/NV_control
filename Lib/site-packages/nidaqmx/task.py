...
from nidaqmx._task_modules.read_functions import (
    ...
    _read_counter_u_32_ex, _read_counter_f_64_ex, _read_counter_f_64)
...
class Task(object):
    ...
    # Hoang added - needs checking. Never trust Hoang's codes.
    def set_buffer(self, numSampsPerChan):
        cfunc = lib_importer.windll.DAQmxCfgInputBuffer
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle, ctypes.c_double]

        error_code = cfunc(self._handle, numSampsPerChan)
        check_for_error(error_code)
    ...
    def setup_gated_counter(self, channel, num_samples):
        ...
        self._check_error(nidaq.DAQmxCreateTask("", ctypes.byref(task['task_handle'])))
        ...
        self._check_error(
            nidaq.DAQmxCreateCIPulseWidthChan(task['task_handle'], input_channel_str_gated, '', MIN_TICKS,
                                                   MAX_TICKS, DAQmx_Val_Ticks, DAQmx_Val_Rising, ''))

        # specify number of samples to acquire
        self._check_error(nidaq.DAQmxCfgImplicitTiming(task['task_handle'],
        ...
        self._check_error(nidaq.DAQmxSetCICtrTimebaseSrc(task['task_handle'], input_channel_str_gated,
        ...
        self._check_error(nidaq.DAQmxSetCIPulseWidthTerm(task['task_handle'], input_channel_str_gated,
                                                                  gate_PFI_str))
        ...
        # for more details)
        self._check_error(
            nidaq.DAQmxSetCIDupCountPrevent(task['task_handle'], input_channel_str_gated, bool32(True)))
