...
class SCCRRPhotonStatChargeCheckShelve(Instrument):

    def __init__(self, name='SCCRRPhotonStatChargeCheckShelveObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        self.LaserGreenLowParam =   {'delay_time': 2, 'channel':settings['laserGreenLow_channel']}
        ...
        settings_extra = {'clock_speed': self.clock_speed, 'Counter': self.CounterParam, 'AWG': self.AWGParam,
                          ...
                          'LaserShelve':self.LaserShelveParam, 'LaserGreenLow':self.LaserGreenLowParam,
        ...
        self.add_parameter(
            name = "check1",
            parameter_class = Check1,
        )
        self.add_parameter(
            name = "check2",
            parameter_class = Check2,
        )
        self.add_parameter(
            name = "check1_ref",
            parameter_class = Check1Ref,
        )
        self.add_parameter(
            name = "check2_ref",
            parameter_class = Check2Ref,
        )
    ...
    def runScan(self):
        ...
        check1 = self.check1
        check2 = self.check2
        check1_ref = self.check1_ref
        check2_ref = self.check2_ref
        ...
        loop = Loop(
            ...
            sleepTimeAfterFinishing=0).each(sig,ref, sigFullData, refFullData, check1,check2,check1_ref,check2_ref,
                                            qctask(sig.plotPulseSequences),
                                            )
        data = loop.get_data_set(name='SCCRRPhotonStatChargeCheckShelve')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/SCCRRPhotonStatChargeCheckShelveObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.SCCRRPhotonStatChargeCheckShelveObject = measurementObject
        self.loopCounter = 0
        ...
        self.ifIonRR = self.settings['ifIonRR']
        self.ifGreenLow = self.settings['ifGreenLow']
    ...
    def set_raw(self, tau_ns):
        ...
        pi_to_shelve_delay      = self.settings['pi_to_shelve_delay'];     shelve_duration        = self.settings['shelve_duration']
        ...
        iznLaserSwitch_delay    = self.settings['iznLaserSwitch_delay'];   greenLaserSwitch_delay = self.settings['greenLaserSwitch_delay']
        ...
        amp_MW_mix              = self.settings['amp_MW_mix'];             check_duration         = self.settings['check_duration']

        if sweepWhich == 'ti':
            ion_duration         = tau_ns
        elif sweepWhich == 'i2r':
            ion_to_read_delay    = tau_ns
        elif sweepWhich == 'tr':
            DAQ_duration         = tau_ns
            ...
            print('delay_between_MWmix', delay_between_MWmix)
            ...
            srs2 = self.SCCRRPhotonStatChargeCheckShelveObject.srs2
            srs2.set_RFAmplitude(MWPower2) #dBm
        elif sweepWhich =='tsh':
            shelve_duration = tau_ns
        elif sweepWhich =='sh2i':
            shelve_to_ion_delay = tau_ns
        ...
        ######### Charge check ##################
        check1_delay            = laser_init_delay + laser_to_pi_delay
        check2_delay            = when_init_end - check_duration
        ######### The spin init block ###########
        ...
        shelve_delay            = when_pi_end + pi_to_shelve_delay
        when_ion_start          = shelve_delay + shelve_duration + shelve_to_ion_delay + greenLaserSwitch_delay
        ion_RR_delay            = when_ion_start - RRLaserSwitch_delay        
        when_ion_RR_end         = ion_RR_delay + ion_duration
        ion_strong_delay        = when_ion_start - iznLaserSwitch_delay
        when_ion_strong_end     = ion_strong_delay + ion_duration
        hilo_duration2          = when_ion_strong_end-shelve_delay
        ...
        hilo_duration              = MWmix_duration + margin + laser_init_delay

        green_low_delay            = pi_delay #laser_init_delay 
        green_low_duration         = when_laser_read_signal_end - green_low_delay #laser_init_duration #
        ###################################################################################################################################
        ###################################################################################################################################
        ###################################################################################################################################
        ...
        ######### Charge check ##################
        check1_ref_delay            = laser_init_ref_delay + laser_to_pi_delay
        check2_ref_delay            = when_init_ref_end - check_duration
        ######### The spin init block ###########
        ...
        shelve_ref_delay        = when_pi_ref_end + pi_to_shelve_delay
        when_ion_ref_start      = shelve_ref_delay + shelve_duration + shelve_to_ion_delay + greenLaserSwitch_delay
        ion_ref_RR_delay        = when_ion_ref_start - RRLaserSwitch_delay
        when_ion_ref_RR_end     = ion_ref_RR_delay + ion_duration
        ion_ref_strong_delay    = when_ion_ref_start - iznLaserSwitch_delay
        ...
        hilo_ref_duration       = MWmix_ref_duration + margin + laser_init_delay

        green_low_ref_delay     = pi_ref_delay #laser_init_ref_delay #
...
##########################################################################################################
        ...
        pulse_sequence += [spc.Pulse('Counter',      check1_delay,            duration=int(check_duration))] 
        pulse_sequence += [spc.Pulse('Counter',      check2_delay,            duration=int(check_duration))] 
        ...
        if shelve_duration > 0:
            pulse_sequence += [spc.Pulse('LaserShelve',  shelve_delay,          duration=int(shelve_duration))]
            if self.ifGreenLow==1:
                pulse_sequence += [spc.Pulse('LaserGreenLow',green_low_delay,       duration=int(green_low_duration))]
        ...
        if self.ifHiloExtra==1:
            ...
            pulse_sequence += [spc.Pulse('hiLoMWPwr',    shelve_delay,   duration=int(hilo_duration2))]
        ...
        pulse_sequence += [spc.Pulse('Counter',      check1_ref_delay,            duration=int(check_duration))] 
        pulse_sequence += [spc.Pulse('Counter',      check2_ref_delay,            duration=int(check_duration))]
        ...
        if shelve_duration > 0:
            pulse_sequence += [spc.Pulse('LaserShelve',  shelve_ref_delay,     duration=int(shelve_duration))]
            if self.ifGreenLow == 1:
              pulse_sequence += [spc.Pulse('LaserGreenLow',green_low_ref_delay,       duration=int(green_low_duration))]
        ...
        if self.ifHiloExtra==1:
            ...
            pulse_sequence += [spc.Pulse('hiLoMWPwr', shelve_ref_delay, duration=int(hilo_duration2))] 
    ...
    def get_raw(self):
        ...
        global sig_data;   sig_data   = rate[2::self.num_reads_per_iter]
        global ref_data;   ref_data   = rate[5::self.num_reads_per_iter]
        global check1;     check1     = rate[::self.num_reads_per_iter]
        global check2;     check2     = rate[1::self.num_reads_per_iter]
        global check1_ref; check1_ref = rate[3::self.num_reads_per_iter]
        global check2_ref; check2_ref = rate[4::self.num_reads_per_iter]
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.SCCRRPhotonStatChargeCheckShelveObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
...
class Check1(Parameter):
    def __init__(self, name='check1',**kwargs):
        super().__init__(name, **kwargs)

    def get_raw(self):
        return check1

class Check2(Parameter):
    def __init__(self, name='check2',**kwargs):
        super().__init__(name, **kwargs)

    def get_raw(self):
        return check2

class Check1Ref(Parameter):
    def __init__(self, name='check1_ref',**kwargs):
        super().__init__(name, **kwargs)

    def get_raw(self):
        return check1_ref

class Check2Ref(Parameter):
    def __init__(self, name='check2_ref',**kwargs):
        super().__init__(name, **kwargs)
