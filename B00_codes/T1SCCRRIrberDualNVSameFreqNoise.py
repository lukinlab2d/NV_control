...
class T1SCCRRIrberDualNVSameFreqNoise(Instrument):

    def __init__(self, name='T1SCCRRIrberDualNVSameFreqNoiseObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        settings_extra = {'clock_speed': self.clock_speed, 'Counter': self.CounterParam, 
                          'AWG': self.AWGParam,'noise': self.noiseParam,
        ...
        self.srate=self.settings['srate']; self.ifIQ = self.settings['ifIQ']
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T1SCCRRIrberDualNVSameFreqNoise')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T1SCCRRIrberDualNVSameFreqNoiseObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.T1SCCRRIrberDualNVSameFreqNoiseObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        spinInit_pi_to_RR_delay= self.settings['spinInit_pi_to_RR_delay']; initStateRef           = self.settings['initStateRef']
        ...
        amp_MW_mix              = self.settings['amp_MW_mix'];             ifGHzNoise             = self.settings['ifGHzNoise']
        pi_pulse_bf_read        = self.settings['pi_pulse_bf_read'];       pi_time3               = self.settings['pi_time3']
        ifIznNV2later           = self.settings['ifIznNV2later'];          initStateSig           = self.settings['initStateSig']
        if sweepWhich == 'ti':
            ion_duration         = tau_ns
            ion_duration2        = tau_ns
        elif sweepWhich == 'tinit':
            laser_init_duration  = tau_ns
        else:
            pi_to_ion_delay = int(tau_ns)
            # pi_to_hilo_extra_delay = int(tau_ns)
        ...
        noise_delay             = when_pi_end2 + pi_to_hilo_extra_delay
        noise_duration          = pi_to_ion_delay - pi_to_hilo_extra_delay
        ...
        if ifIznNV2later==1:
            ion_RR2_delay       = ion_RR2_delay + ion_duration + 1e3
            when_ion_RR2_end    = ion_RR2_delay + ion_duration2
            ion_strong_duration = when_ion_RR2_end - ion_strong_delay
            when_ion_strong_end = ion_strong_delay + ion_strong_duration
        ...
        DAQ_signal_delay           = laser_read_signal_delay
        ...
        MWmix_duration_both = when_MWmix_NV2_end - MWmix_delay
        ...
        noise_ref_delay         = when_pi_ref_end2 + pi_to_hilo_extra_delay
        ...
        if ifIznNV2later==1:
            ion_ref_RR2_delay       = ion_ref_RR2_delay + ion_duration + 1e3
        ...
        DAQ_ref_delay           = laser_read_ref_delay
        ...
        sig_to_ref_wait_revised = pi_ref_delay - when_MWmix_NV2_end
        ...
        if self.ifAWG:
            ...
            ch1plot, ch2plot = AWG.send_T1SCCRRSameFreqNoise_seq(int(MWmix_duration_both), int(AWG_buffer),
                            int(sig_to_ref_wait_revised), pitime=int(pi_time), pi_to_MWmix_wait=int(pi_to_MWmix_wait),
                            sleepTime=sleepTime,amp_MW_mix=amp_MW_mix,initStateSig=initStateSig,initStateRef=initStateRef)
...
##########################################################################################################
        ...
        # Ionization
        # add noise in "sig" only if start in 0/1 for sig/ref, otherwise "sig" is no noise
        if ifGHzNoise==1 and initStateRef!=initStateSig: 
        ...
        if self.ifMWDuringRead:
            if self.ifMWReadLowDutyCycle:
                ...
                if self.ifAWG == 0:
                    pulse_sequence += [spc.Pulse('MWswitch', MWmix_NV2_delay,         duration=int(MWmix_duration))]
        if self.ifMW2DuringRead:
            if self.ifMWReadLowDutyCycle:
                for i in range(nMWread):
                    delay = MWmix_NV2_delay + i*(MWmix_duration_short + delay_between_MWmix)
                ...
                pulse_sequence += [spc.Pulse('MWswitch3',MWmix_NV2_delay,         duration=int(MWmix_duration))]
...
##############################################################################################################################
        ...
        # Ionization
        if ifGHzNoise==1:
            pulse_sequence += [spc.Pulse('noise',      noise_ref_delay,          duration=int(noise_duration))] 
        ...
        if self.ifMWDuringRead:
            if self.ifMWReadLowDutyCycle:
                ...
                if self.ifAWG == 0:
                    pulse_sequence += [spc.Pulse('MWswitch', MWmix_ref_NV2_delay,  duration=int(MWmix_ref_duration))]
        if self.ifMW2DuringRead:
            if self.ifMWReadLowDutyCycle:
                for i in range(nMWread):
                    delay = MWmix_ref_NV2_delay + i*(MWmix_duration_short + delay_between_MWmix)
                ...
                pulse_sequence += [spc.Pulse('MWswitch3',MWmix_ref_NV2_delay,  duration=int(MWmix_ref_duration))]
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T1SCCRRIrberDualNVSameFreqNoiseObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
