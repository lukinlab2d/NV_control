...
class T2EDEER(Instrument):

    def __init__(self, name='T2EDEERObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T2EDEER')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.T2EDEERObject_sig, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig'
            )
        plot.add(data.T2EDEERObject_ref, name='ref')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T2EDEERObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        ...
        self.T2EDEERObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        ch1plot2, ch2plot2 = AWG2.send_DEER_seq(pi_2time=int(pi2time), pitime = int(pitime), tau = int(tau_ns/2),
                                             buffer=int(AWG_buffer), sig_to_ref_wait=int(sig_to_ref_wait),
                                             sleepTime=sleepTime)
    ...
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T2EDEERObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
