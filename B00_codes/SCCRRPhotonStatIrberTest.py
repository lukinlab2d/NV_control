...
for whichNV in np.linspace(2,2,1):
    ...
    if np.mod(whichNV,2)==1: 
        ...
        SRSnum2= 3; MWPower2 = 6.00; pi_time2 = -1; MWFreq2 = 3161.27e6   #NV D1 ms+1
        MWI_channel  = 1; MWQ_channel  = 0;  MWswitch_channel  = 2
        MWI2_channel = 0; MWQ2_channel = 0;  MWswitch2_channel = 15; amp_MW_mix = 0.95
        SDGnum       = 1; AWG_channel  = 18; srate             = 2.5e8
    else:
        velNum = 2; vel_current = 67.1; vel_wvl = 636.88; vel_vpz_target=-1; laserRead_channel = 14
        SRSnum = 2; MWPower  = -9.8; pi_time  = 40; MWFreq = 2788.70e6   #NV D2, ms-1
        ...
        MWI2_channel = 0;  MWQ2_channel = 0;  MWswitch2_channel = 16; amp_MW_mix = 0.95
        SDGnum       = 2;  AWG_channel  = 19; srate             = 2.5e8
    
    # tausArray = np.array((5e2,1e3,2e3,3e3,4e3,5e3,6e3,7e3,8e3,10e3,12e3)) # sweep ti
    # tausArray = np.array((5e5,1e6,2e6,2.5e6,3e6,4e6,5e6)) # sweep tr
    # tausArray = np.linspace(28,44,5)         # rabi
    tausArray = np.array((2e6,3e6,5e6,7e6,8.5e6,10e6,12e6))#,15e6,20e6)) # tinit
    ...
    # tausArray = np.linspace(-1,3,5) # amp_MW_mix / MWPower2
    num_loops                    = int(1.5e3); nSpinInit                 = 0
    laser_init_delay             = 1e2;        laser_init_duration       = int(5e6)
    ...
    pi_to_ion_delay              = 1e2;        ion_duration              = 8e3
