...
from B00_codes.AutoCorrXY8RRDualNV import *
import time, dropbox
...
for i in np.linspace(1,reps,reps):
    ...
    # AutoCorrXY8RRDualNV 
    offset = 0; tausArray = np.linspace(offset,offset+5120,65)

    num_loops                    = int(3e5);   NXY8                   = 3
    laser_init_delay             = 1e2;        laser_init_duration    = 8e3
    MW_to_read_delay             = 1e2;        MWI_to_switch_delay    = 30
    laser_to_DAQ_delay_directory = {3: 850, 6: 1150, 9: 1150, 7: 900, 5: 1650, 14:800}
    ...
    settings = {'tausArray': tausArray, 'num_loops':num_loops, 'MWPower':MWPower, 'MWFreq': MWFreq, 'SRSnum': SRSnum,
                ...
                'ifSweepPhaseLastPiHalf':ifSweepPhaseLastPiHalf,'phi_IQ':phi_IQ, 'tau':tau, 'phi_IQ2':phi_IQ2, 
    ...
    #################################
    start = time.time()
    AutoCorrXY8RRDualNVObject = AutoCorrXY8RRDualNV(settings=settings, ifPlotPulse=not(ifLooped)) 
    AutoCorrXY8RRDualNVObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    AutoCorrXY8RRDualNVObject.close()
