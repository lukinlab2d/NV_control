...
class ScanROPower(Instrument):

    def __init__(self, name='ScanROPowerObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        self.velCurrentArray = self.settings['velCurrentArray']
        self.SRSnum = self.settings['SRSnum'];      MWPower = self.settings['MWPower']; MWFreq = self.settings['MWFreq']
        vel_vpz = self.settings['vel_vpz']; vel_wvl = self.settings['vel_wvl'] 
        ...
        time.sleep(1)
        self.vel.set_ready()
        self.vel.set_current(30)
        self.vel.set_vpiezo(vel_vpz)
        self.vel.set_ready()
    ...
    def runScan(self):
        ...
        loop = Loop(
            sig.sweep(keys=self.velCurrentArray),
            delay = 0,
            sleepTimeAfterFinishing=0).each(sig,ref,sigOverRef,
                                            ).then(qctask(sig.close_turnOnAtEnd))

        data = loop.get_data_set(name='ScanROPower')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.ScanROPowerObject_sig, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig'
            )
        plot.add(data.ScanROPowerObject_ref, name='ref')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/ScanROPowerObject_sig_set.dat'
...
class Signal(Parameter):
    ...
    def set_raw(self, current):
        vel.set_current(current)
        vel.set_ready()
        time.sleep(0.5)
        print("Loop " + str(self.loopCounter))
        print("Set current to " + str(np.round(current,1)) + ' mA') # set the Velocity's current
