...
class T2ESCC(Instrument):

    def __init__(self, name='T2ESCCObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T2ESCC')
        data.add_metadata(self.settings)
        self.data = data

        # plot = QtPlot(
        #     data.T2ESCCObject_sig, # this is implemented as a Parameter
        #     figsize = (1200, 600),
        #     interval = 1,
        #     name = 'sig'
        #     )
        # plot.add(data.T2ESCCObject_ref, name='ref')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T2ESCCObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        ...
        self.T2ESCCObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        laser_to_MWS_delay      = self.settings['laser_to_MWS_delay'];     pi_half             = self.settings['pi_half']; pi = 2*pi_half
        MWI_to_switch_delay     = self.settings['MWI_to_switch_delay']
        MWS_to_shelve_delay     = self.settings['MWS_to_shelve_delay'];    shelve_duration     = self.settings['shelve_duration']  
        ...
        MWS_delay              = when_init_end          + laser_to_MWS_delay;     when_MWS_end              = MWS_delay        + pi_half
        MWS2_delay             = when_MWS_end           + tau_ns/2;               when_MWS2_end             = MWS2_delay       + pi
        MWS3_delay             = when_MWS2_end          + tau_ns/2;               when_MWS3_end             = MWS3_delay       + pi_half
        MWI_delay              = MWS3_delay             - MWI_to_switch_delay;    when_MWI_end              = MWI_delay        + pi_half + MWI_to_switch_delay
        MWQ_delay              = MWI_delay;                                       when_MWQ_end              = when_MWI_end
        shelve_delay           = when_MWS3_end          + MWS_to_shelve_delay;    when_shelve_end           = shelve_delay     + shelve_duration
        ...
        pulse_sequence         += [spc.Pulse('MWswitch',     MWS_delay,              duration=int(pi_half))]
        pulse_sequence         += [spc.Pulse('MWswitch',     MWS2_delay,             duration=int(pi))]
        pulse_sequence         += [spc.Pulse('MWswitch',     MWS3_delay,             duration=int(pi_half))]
        ...
        ###################################################################################################################
        if self.ifMeaningfulRef:
            pulse_sequence     += [spc.Pulse('LaserInit',    laser_init_delay       + total_sig_duration, duration=int(laser_init_duration))] 
            pulse_sequence     += [spc.Pulse('MWswitch',     MWS_delay              + total_sig_duration, duration=int(pi_half))]
            pulse_sequence     += [spc.Pulse('MWswitch',     MWS2_delay             + total_sig_duration, duration=int(pi))]
            if normalized_style == Q_FINAL:
                pulse_sequence += [spc.Pulse('MW_I',         MWI_delay              + total_sig_duration, duration=int(pi_half + 2*MWI_to_switch_delay))]
                pulse_sequence += [spc.Pulse('MW_Q',         MWQ_delay              + total_sig_duration, duration=int(pi_half + 2*MWI_to_switch_delay))]
                pulse_sequence += [spc.Pulse('MWswitch',     MWS3_delay             + total_sig_duration, duration=int(pi_half))]
                ...
                pulse_sequence += [spc.Pulse('MWswitch',     MWS3_delay             + total_sig_duration, duration=int(pi_half + pi))] # this affects the shelving pulse
                
                if not shelve_duration == 0:
                    pulse_sequence += [spc.Pulse('LaserShelve',  shelve_delay           + total_sig_duration + pi, duration=int(shelve_duration))]
                pulse_sequence     += [spc.Pulse('LaserIon',     ion_delay              + total_sig_duration + pi, duration=int(ion_duration))]
                pulse_sequence     += [spc.Pulse('LaserRead',    laserRead_signal_delay + total_sig_duration + pi, duration=int(laserRead_signal_duration))] 
                pulse_sequence     += [spc.Pulse('Counter',      DAQ_signal_delay       + total_sig_duration + pi, duration=int(DAQ_duration))] 
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T2ESCCObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
