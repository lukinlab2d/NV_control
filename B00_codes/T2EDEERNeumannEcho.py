...
class T2EDEERNeumannEcho(Instrument):

    def __init__(self, name='T2EDEERNeumannEchoObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        settings_extra = {'clock_speed': self.clock_speed, 'Counter': self.CounterParam, 
                        ...
                        'AWG': self.AWGParam, 'PB_type': 'USB', 'AWG2': self.AWGParam2,
        ...
        self.SRSnum2 =self.settings['SRSnum2']; self.uwPower2 = self.settings['uwPower2']; self.uwFreq2 = self.settings['uwFreq2']
        ...
        self.add_parameter(
            name = "sig_echo",
            parameter_class = SignalEcho,
        )
        self.add_parameter(
            name = "ref_echo",
            parameter_class = ReferenceEcho,
        )
        ...
        self.srs2.set_freq(self.uwFreq2) #Hz
        self.srs2.set_RFAmplitude(self.uwPower2) #dBm
    ...
    def runScan(self):
        ...
        sig_echo = self.sig_echo
        ref_echo = self.ref_echo
        ...
        loop = Loop(
            ...
            delay = 0,
            sleepTimeAfterFinishing=0).each(sig, ref, sig_echo, ref_echo,
                                            qctask(sig.plotPulseSequences),
                                            ).then(qctask(sig.turn_on_at_end))

        data = loop.get_data_set(name='T2EDEERNeumannEcho')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.T2EDEERNeumannEchoObject_sig, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig'
            )
        plot.add(data.T2EDEERNeumannEchoObject_ref, name='ref')
        plot.add(data.T2EDEERNeumannEchoObject_sig_echo, name='sig_echo')
        plot.add(data.T2EDEERNeumannEchoObject_ref_echo, name='ref_echo')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T2EDEERNeumannEchoObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        ...
        self.T2EDEERNeumannEchoObject = measurementObject
        self.loopCounter = 0
    ...
    def get_raw(self):
        ...
        sig_echo = rate[2::self.num_reads_per_iter]
        ref_echo = rate[3::self.num_reads_per_iter]
        global sig_avg;  sig_avg = np.average(sig)
        global ref_avg;  ref_avg = np.average(ref)
        global sig_echo_avg;  sig_echo_avg = np.average(sig_echo)
        global ref_echo_avg;  ref_echo_avg = np.average(ref_echo)
    ...
    def set_raw(self, tau_ns):
        ...
        tau_echo                = self.settings['tau_echo'];               pitimeDEER          = self.settings['pitimeDEER']
        MW_duration = int(2*int((AWG_buffer + 2*pi2time + pitime + tau_echo + 1)/2))
        ...
        laser_read_signal_echo_delay = when_laser_read_ref_end + laser_read_signal_delay
        read_signal_echo_delay = laser_read_signal_echo_delay + laser_to_DAQ_delay
        when_laser_read_signal_echo_end = laser_read_signal_echo_delay + laser_read_signal_duration

        laser_read_ref_echo_delay = when_laser_read_signal_echo_end + laser_read_signal_delay
        read_ref_echo_delay = laser_read_ref_echo_delay + laser_to_DAQ_delay
        ...
        ch1plot, ch2plot = AWG.send_T2ETwice_seq(pi_2time=int(pi2time), pitime = int(pitime), tau = int(tau_echo/2),
        ...
        ch1plot2, ch2plot2 = AWG2.send_DEERNeumann_seq(pi_2time=int(pi2time), pitime = int(pitime), tau = int(tau_echo/2),
                                                       T=tau_ns, pitimeDEER = int(pitimeDEER),
                                             buffer=int(AWG_buffer), sig_to_ref_wait=int(sig_to_ref_wait),
                                             sleepTime=sleepTime, ifEcho=1)
        ...
        pulse_sequence += [spc.Pulse('AWG2',      MW_delay,                duration=100)]
        ...
        pulse_sequence += [spc.Pulse('LaserRead', laser_read_signal_echo_delay, duration=int(laser_read_signal_duration))] # times are in ns
        pulse_sequence += [spc.Pulse('LaserRead', laser_read_ref_echo_delay,    duration=int(laser_read_ref_duration))]
        ...
        pulse_sequence += [spc.Pulse('Counter',   read_signal_echo_delay,       duration=int(read_signal_duration))] # times are in ns
        pulse_sequence += [spc.Pulse('Counter',   read_ref_echo_delay,          duration=int(read_ref_duration))] # times are in ns
    ...
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T2EDEERNeumannEchoObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
...
class SignalEcho(Parameter):
    def __init__(self, name='sig_echo',**kwargs):
        super().__init__(name, **kwargs)

    def get_raw(self):
        return sig_echo_avg

class ReferenceEcho(Parameter):
    def __init__(self, name='ref_echo',**kwargs):
        super().__init__(name, **kwargs)
