...
ifInitWvl=0; ifInitVpz=0; ifNeedVel1=0; ifNeedVel2=0; if_tracking=0
ifHiloExtra=1; ifRandomized=0; ifAWG=1; srate=2.5e8
ifTestSig=0; ifRndPhaseNoise=0; AGBW = 50e3; AGfreq = 1506024; AGamp = 0*0.2 # beware of heating!!
for i in np.linspace(1, reps, reps):
    # T2ERRDualNV_NewTrack
    tausArray = np.round(np.logspace(np.log10(1e2),np.log10(1e6),15),-1)
    # tausArray = np.linspace(100,10100,101)

    num_loops                    = int(5e4); phi_IQ = 0.48*pi; phi_IQ2 = 0.49*pi
    laser_init_delay             = 5e2;        laser_init_duration    = 30e3
    MW_to_read_delay             = 40;         MWI_to_switch_delay    = 30
    ...
    if True:
        ...
        SRSnum  = 1; MWPower  = -6.2;  pi_half  = 20; MWFreq   = 2598.44e6   #NV D1 ms-1
