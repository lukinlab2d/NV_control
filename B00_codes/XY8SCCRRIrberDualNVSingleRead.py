...
class XY8SCCRRIrberDualNVSingleRead(Instrument):

    def __init__(self, name='XY8SCCRRIrberDualNVSingleReadObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='XY8SCCRRIrberDualNVSingleRead')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/XY8SCCRRIrberDualNVSingleReadObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.XY8SCCRRIrberDualNVSingleReadObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        MW_delays.append(firstPiHalf_delay)  
        when_firstPiHalf_end    = firstPiHalf_delay + pi_time/2
        pi_delay                = when_firstPiHalf_end + tau_ns/2
        MW_delays.append(pi_delay) 
        for i in range(2,NXY8*8+1):
            MW_delays.append(MW_delays[i-1] + pi_time + tau_ns)
        MW_delays.append(MW_delays[NXY8*8] + pi_time + tau_ns/2)
        when_pulse_end          = MW_delays[NXY8*8+1] + pi_time/2
        pulse_duration          = (NXY8*8)*tau_ns + (NXY8*8+1)*pi_time
        ################## Ionization #######################
        ion_RR_delay            = when_pulse_end + shift_btwn_2NV_MW + pi_to_ion_delay                 
        ...
        MW_ref_delays.append(firstPiHalf_ref_delay)  
        when_firstPiHalf_ref_end    = firstPiHalf_ref_delay + pi_time/2
        pi_ref_delay                = when_firstPiHalf_ref_end + tau_ns/2
        MW_ref_delays.append(pi_ref_delay) 
        for i in range(2,NXY8*8+1):
            MW_ref_delays.append(MW_ref_delays[i-1] + pi_time + tau_ns)
        MW_ref_delays.append(MW_ref_delays[NXY8*8] + pi_time + tau_ns/2)
        when_pulse_ref_end          = MW_ref_delays[NXY8*8+1] + pi_time/2
        ################## Ionization #######################
        ion_ref_RR_delay        = when_pulse_ref_end + shift_btwn_2NV_MW + pi_to_ion_delay
...
############################################################################################################################################################################
        ...
        pulse_sequence += [spc.Pulse('MWswitch',     MW_delays[NXY8*8+1],               duration=int(pi_time/2))]
        ...
        pulse_sequence += [spc.Pulse('MWswitch2',     MW_delays[NXY8*8+1] + shift_btwn_2NV_MW,               duration=int(pi_time/2))]

        if self.ifSinDetect:
            pulse_sequence += [spc.Pulse('MW_I', MW_delays[NXY8*8+1] - MWI_to_switch_delay, duration=int(pi_time/2 + 2*MWI_to_switch_delay))]
            pulse_sequence += [spc.Pulse('MW_I2',MW_delays[NXY8*8+1] + shift_btwn_2NV_MW - MWI_to_switch_delay, duration=int(pi_time/2 + 2*MWI_to_switch_delay))]
...
#############################################################################################################################################################################################
        ...
        if normalized_style == Q_FINAL:
            pulse_sequence += [spc.Pulse('MWswitch',  MW_ref_delays[NXY8*8+1],                      duration=int(pi_time/2))]
            pulse_sequence += [spc.Pulse('MWswitch2', MW_ref_delays[NXY8*8+1] + shift_btwn_2NV_MW,  duration=int(pi_time/2))]
            
            if self.ifJustRef_CorrACorr == 0: # if just measure ref, turn IQ on opposite to when measuring signal
                if self.ifSinDetect:
                    pulse_sequence += [spc.Pulse('MW_Q',  MW_ref_delays[NXY8*8+1]-MWI_to_switch_delay,                     duration=int(pi_time/2 + 2*MWI_to_switch_delay))]
                    pulse_sequence += [spc.Pulse('MW_Q2', MW_ref_delays[NXY8*8+1]-MWI_to_switch_delay + shift_btwn_2NV_MW, duration=int(pi_time/2 + 2*MWI_to_switch_delay))]
                else:
                    pulse_sequence += [spc.Pulse('MW_I',  MW_ref_delays[NXY8*8+1]-MWI_to_switch_delay,                     duration=int(pi_time/2 + 2*MWI_to_switch_delay))]
                    pulse_sequence += [spc.Pulse('MW_Q',  MW_ref_delays[NXY8*8+1]-MWI_to_switch_delay,                     duration=int(pi_time/2 + 2*MWI_to_switch_delay))] # times are in ns
                    pulse_sequence += [spc.Pulse('MW_I2', MW_ref_delays[NXY8*8+1]-MWI_to_switch_delay + shift_btwn_2NV_MW, duration=int(pi_time/2 + 2*MWI_to_switch_delay))]
                    pulse_sequence += [spc.Pulse('MW_Q2', MW_ref_delays[NXY8*8+1]-MWI_to_switch_delay + shift_btwn_2NV_MW, duration=int(pi_time/2 + 2*MWI_to_switch_delay))] # times are in ns
            else: # if measure cor/anticor, turn IQ on similarly to when measuring signal
                if self.ifSinDetect:
                    pulse_sequence += [spc.Pulse('MW_I',  MW_ref_delays[NXY8*8+1]-MWI_to_switch_delay,                     duration=int(pi_time/2 + 2*MWI_to_switch_delay))]
                    pulse_sequence += [spc.Pulse('MW_I2', MW_ref_delays[NXY8*8+1]-MWI_to_switch_delay + shift_btwn_2NV_MW, duration=int(pi_time/2 + 2*MWI_to_switch_delay))]

        elif normalized_style == THREE_PI_HALF_FINAL:
            pulse_sequence += [spc.Pulse('MWswitch',  MW_ref_delays[NXY8*8+1],                     duration=int(3*pi_time/2))]
            pulse_sequence += [spc.Pulse('MWswitch2', MW_ref_delays[NXY8*8+1] + shift_btwn_2NV_MW, duration=int(3*pi_time/2))]
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.XY8SCCRRIrberDualNVSingleReadObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
