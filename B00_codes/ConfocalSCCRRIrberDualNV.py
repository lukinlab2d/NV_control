...
class ConfocalSCCRRIrberDualNV(Instrument):

    def __init__(self, name='ConfocalSCCRRIrberDualNVObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loopy.get_data_set(name='ConfocalSCCRRIrberDualNV')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/ConfocalSCCRRIrberDualNVObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.ConfocalSCCRRIrberDualNVObject = measurementObject
        self.loopCounter = 0
        ...
        shift_btwn_2NV_read    = self.settings['shift_btwn_2NV_read']
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.xArray)-1: # only save first and last pulse sequence
                self.ConfocalSCCRRIrberDualNVObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
