...
from B00_codes.T1SCCRRIrberDualNVSameFreqNoise import *
from qcodes_contrib_drivers.drivers.Agilent.Agilent_33522A import AG33522A
###########################################################################################e#########################
reps = 1e5
for iii in range(int(reps)):
    # T1SCCRRIrberDualNVSameFreqNoise
    ifRandomized = 1; ifPlotPulse=(reps==-1); ifMWReadLowDutyCycle=0; ifNeedVel=0; ifIznNV2later=0
    ifFancySpinInit = 0; sweepWhich = 'pi_to_ion_delay'; noise_channel = 2; ifGHzNoise=1
    ifMWDuringRead=1; ifMW2DuringRead=1; ifIQ=ifAWG=1; ifHiloExtra=1; initStateSig=1; initStateRef=1
    laserInit_channel=3; laserIon_channel=10; hiLoMWPwr_channel=17; ifInitVpz=0; ifInitWvl=0
    ifAG33522A = 1; AGBW = 1e7; AGfreq=AGBW; AGamp = 3.8 # beware of heating!!
    ...
    # tausArray = np.linspace(5e3,160e3,32)
    t1 = np.round(np.logspace(np.log10(1.2e3),np.log10(3e6),15),-1)
    t2 = np.array([ 820.,  840.,  870.,  920., 1020, 1200])
    tausArray = t2#np.concatenate((t1,t2))
    ##############################################################################################################
    if True:
        # NV1
    ...
    settings = {'tausArray': tausArray, 'noise_channel':noise_channel, 
                'initStateSig':initStateSig,'initStateRef':initStateRef,'ifGHzNoise':ifGHzNoise,
                ...
                'amp_MW_mix':amp_MW_mix,'srate':srate,'ifIznNV2later':ifIznNV2later
                }
    ...
    #################################
    start = time.time()
    T1SCCRRIrberDualNVSameFreqNoiseObject = T1SCCRRIrberDualNVSameFreqNoise(settings=settings, ifPlotPulse=ifPlotPulse) # this is implemented as an Instrument
    T1SCCRRIrberDualNVSameFreqNoiseObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    
    dataFilename = T1SCCRRIrberDualNVSameFreqNoiseObject.getDataFilename()
    T1SCCRRIrberDualNVSameFreqNoiseObject.close()
