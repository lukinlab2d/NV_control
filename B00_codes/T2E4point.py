...
class T2E4point(Instrument):

    def __init__(self, name='T2E4pointObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        loop = Loop(
            ...
            delay = 0,
            sleepTimeAfterFinishing=0).each(sig, ref, ref2, ref3,
                                            qctask(sig.plotPulseSequences),
                                            ).then(qctask(sig.turn_on_at_end))

        data = loop.get_data_set(name='T2E4point')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.T2E4pointObject_sig, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig'
            )
        plot.add(data.T2E4pointObject_ref, name='ref')
        plot.add(data.T2E4pointObject_ref2, name='ref2')
        plot.add(data.T2E4pointObject_ref3, name='ref3')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T2E4pointObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        ...
        self.T2E4pointObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        AWG_buffer              = self.settings['AWG_buffer'];              phi_3rdRead         = self.settings['phi_3rdRead']
        ...
        I = round(32767*np.cos(phi_3rdRead))
        Q = round(32767*np.sin(phi_3rdRead))
        ...
        ch1plot, ch2plot = AWG.send_T2E4point_seq(pi_2time=int(pi2time), pitime = int(pitime), tau = int(tau_ns/2),
    ...
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T2E4pointObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
