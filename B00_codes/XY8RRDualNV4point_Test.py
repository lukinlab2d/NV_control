...
from B00_codes.XY8RRDualNV4point import *
import time, dropbox
...
for i in np.linspace(1,reps,reps):
    # XY8RRDualNV4point 
    ifLooped=(reps!=1); laserInit_channel=3; normalized_style=Q_FINAL; hiLoMWPwr_channel=17
    ifInitWvl=0; ifInitVpz=0; ifNeedVel1=0; ifNeedVel2=0; if_tracking=0; ifStartInY=0
    ifHiloExtra=1; ifRandomized=0; sweepWhich='phase_last_pi_half'
    ...
    tausArray = np.linspace(0*pi, 2.0*pi, 26)
    # f = np.linspace(2e6,2.4e6,201)
    # t = 1/(2*f)*1e9 - 40; t = (4*np.rint(t/4)); tausArray = np.unique(t)

    num_loops                    = int(0.8e5);   NXY81 = 5; NXY82 = 8
    laser_init_delay             = 5e2;        laser_init_duration    = 45e3
    MW_to_read_delay             = 1e2;        MWI_to_switch_delay    = 30
    laser_to_DAQ_delay_directory = {3: 860, 6: 1150, 9: 1150, 7: 900, 5: 1650, 14:900}
    ...
    shift_btwn_2NV_MW            = 40;         shift_btwn_2NV_read    = read_duration + 1.7e3
    AWG_buffer                   = 20;         AWG_output_delay       = 1448  
    AWG_buffer2                  = 20;         AWG_output_delay2      = 1448 
    phi_IQ = 0.50*pi; phi_IQ2    = 0.50*pi;    tau                    = int(1/(2*AGfreq)*1e9-40)
    tauExtra                     = 0;          srate                  = 2.5e8#None
    hilo_margin_start = 200; hilo_margin_end = 100; hilo_min = 30
    if True:
        ########### NV1 ##############
        velNum = 1; vel_current = 62.7; vel_wvl = 637.22; vel_vpz_target = -1; laserRead_channel = 5
        SRSnum = 1; MWPower     = -6.0; pi_half = 20; pi_pulse = 40;  MWFreq = 2598.44e6   #NV D1 ms-1
        ...
        SRSnum2 = 2; MWPower2   = -9.8; pi_half2 = 20; pi_pulse2 = 40; MWFreq2= 2788.70e6   #NV D2 ms-1
    ...
    settings = {'tausArray': tausArray, 'num_loops':num_loops, 'MWPower':MWPower, 'MWFreq': MWFreq, 'SRSnum': SRSnum,
                ...
                'sweepWhich':sweepWhich,
                'last_pi_2time':last_pi_2time, 'tauExtra':tauExtra,
                'ifTestSig':ifTestSig, 'ifHiloExtra':ifHiloExtra,'srate':srate,
    ...
    #################################
    start = time.time()
    XY8RRDualNV4pointObject = XY8RRDualNV4point(settings=settings, ifPlotPulse=not(ifLooped)) 
    XY8RRDualNV4pointObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    XY8RRDualNV4pointObject.close()
