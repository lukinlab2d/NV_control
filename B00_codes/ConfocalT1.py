...
class ConfocalT1(Instrument):

    def __init__(self, name='ConfocalT1Object', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        self.galvo = DAQAnalogOutputs("ConfocalT1DAQ", galvo_card_name, galvo_ao_channels)
    ...
    def runScan(self):
        ...
        data = loopy.get_data_set(name='ConfocalT1')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/ConfocalT1Object_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, name='sig', settings=None, measurementObject=None, **kwargs):
        ...
        self.loopCounter = 0
        self.ConfocalT1Object = measurementObject
    ...
    def set_raw(self, tau_ns):
        ...
        laser_init_delay        = self.settings['laser_init_delay'];       ifShimon            = self.settings['ifShimon']
        laser_to_AWG_delay      = self.settings['laser_to_AWG_delay'];     laser_init_duration = self.settings['laser_init_duration']
        ...
        if np.mod(self.loopCounter,len(self.ConfocalT1Object.tausArray))==0:    
            print('pi_increment2 = ' + str(pi_increment2))
            print('pi_increment = ' + str(pi_increment))
            print('pi_time = ' + str(pitime))
            print('-----o-o-o-o-o-o-----')

        MW_duration = int(2*int((2*AWG_buffer + pitime + 1)/2))
        ...
        temp = MW_delay - AWG_output_delay
        if temp > 0:
            MW_delay = temp
        else:
            MW_delay = 0 
        ...
        if ifShimon == 0:
            laser_read_signal_delay    = when_sigMW_end + MW_to_read_delay + tau_ns
            ...
            ch1plot, ch2plot = AWG.send_T1_seq(pitime = int(pitime), buffer=int(AWG_buffer), sleepTime=sleepTime)
            ...
            laser_read_signal_delay = when_sigMW_end + tau_ns + MW_to_read_delay
...
class VoltageX(Parameter):
    def __init__(self, name='x', measurementObject=None, settings=None, **kwargs):
        ...
        sig = self.ConfocalT1Object.sig # this is implemented as a Parameter
        ref = self.ConfocalT1Object.ref # this is implemented as a Parameter
        self.loop = Loop(
    ...
    def set_raw(self,x):
        ...
        if Bstray <= self.BThres:
            ...
            ifOnFlake=1
            self.ConfocalT1Object.tausArray = self.tausArray
            self.loop.sweep_values = sig.sweep(keys=self.tausArray)
        else:
            print("Bstray = %.0f $\mu$T. Off flake" % Bstray)
            ifOnFlake=0
            self.ConfocalT1Object.tausArray = self.tausArray2
