...
class ConfocalT2E(Instrument):

    def __init__(self, name='ConfocalT2EObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        self.galvo = DAQAnalogOutputs("ConfocalT2EDAQ", galvo_card_name, galvo_ao_channels)
    ...
    def runScan(self):
        ...
        data = loopy.get_data_set(name='ConfocalT2E')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/ConfocalT2EObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, name='sig', settings=None, measurementObject=None, **kwargs):
        ...
        self.loopCounter = 0
        self.ConfocalT2EObject = measurementObject
    ...
    def set_raw(self, tau_ns):
        ...
        DAQ_error_factor        = self.settings['DAQ_error_factor']
        num_loops               = int(num_loops/DAQ_error_factor); 
        ...
        if np.mod(self.loopCounter,len(self.ConfocalT2EObject.tausArray))==0:    
        ...
        self.num_reads = int(num_loops * num_reads_per_iter *  DAQ_error_factor)
...
class VoltageX(Parameter):
    def __init__(self, name='x', measurementObject=None, settings=None, **kwargs):
        ...
        self.T2EDictFile = self.settings['T2EDictFile']
        self.BThres = self.settings['BThres']
        self.T2EcontrastThres = self.settings['T2EcontrastThres']
        ...
        if self.T2EDictFile is not None:
            with open(self.T2EDictFile, 'rb') as f:
                self.T2EDict = pickle.load(f)

        sig = self.ConfocalT2EObject.sig # this is implemented as a Parameter
        ref = self.ConfocalT2EObject.ref # this is implemented as a Parameter
        sigStd = self.ConfocalT2EObject.sigStd
        refStd = self.ConfocalT2EObject.refStd
        self.loop = Loop(
    ...
    def set_raw(self,x):
        ...
        if self.T2EDictFile is not None:
            T2Econtrast = self.T2EDict.get((np.round(x_current, 2), np.round(y_current, 2)), -1000)
        srs.set_freq(MWfreq)
        ...
        if self.T2EDictFile is not None:
            if T2Econtrast <= self.T2EcontrastThres:
                print("T2Econtrast = %.4f. On flake" % T2Econtrast)
                ifOnFlake=1
                self.ConfocalT2EObject.tausArray = self.tausArray
                self.loop.sweep_values = sig.sweep(keys=self.tausArray)
            else:
                print("T2Econtrast = %.4f. Off flake" % T2Econtrast)
                ifOnFlake=0
                self.ConfocalT2EObject.tausArray = self.tausArray2
