...
class XY8DrivenCorrSCCRRIrberDualNV(Instrument):

    def __init__(self, name='XY8DrivenCorrSCCRRIrberDualNVObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='XY8DrivenCorrSCCRRIrberDualNV')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/XY8DrivenCorrSCCRRIrberDualNVObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.XY8DrivenCorrSCCRRIrberDualNVObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        NXY8                   = self.settings['NXY8'];                   sweepWhich             = self.settings['sweepWhich']
        ...
        pi_time2                = self.settings['pi_time2'];              XY8_to_DC_delay        = self.settings['XY8_to_DC_delay']
        pi = pi_time; pi_half=pi/2
        # MW_duration_for_AWG     = int(2*int((AWG_buffer + 2*pi_half + NXY8*tau_ns*8 + NXY8*pi*8 + tauExtra)/2))
        ...
        ################## XY8 ##########################################################################
        ###### NV1 ######################################
        MWI_delay               = when_spinInit_lastMW_end + spinInit_RR_to_pi_delay
        MW_delays     = []
        ...
        when_pulse_end = MW_delays[NXY8*8+1] + pi_half
        ###### NV2 ######################################
        MWI_delay2              = MWI_delay + shift_btwn_2NV_MW    
        MW_delays2     = []
        MW_delays2.append(MWI_delay2)         
        MW_delays2.append(MW_delays2[0] + pi_half + tau_ns/2)
        for i in range(2,NXY8*8+1):
            MW_delays2.append(MW_delays2[i-1] + pi + tau_ns)
        MW_delays2.append(MW_delays2[NXY8*8] + pi + tau_ns/2 + tauExtra)
        when_pulse_end2 = MW_delays2[NXY8*8+1] + pi_half 
        #################### extra pulse for driven corr ##################
        pi_delay                = np.max((when_pulse_end,when_pulse_end2)) + XY8_to_DC_delay
        when_pi_end             = pi_delay + pulse_time
        pi_delay2               = pi_delay + shift_btwn_2NV_MW
        when_pi_end2            = pi_delay2 + pulse_time2
        ################## Ionization ###################################################################
        ...
        MW_ref_delays2.append(MW_ref_delays2[0] + pi_half + tau_ns/2)
        for i in range(2,NXY8*8+1):
            MW_ref_delays2.append(MW_ref_delays2[i-1] + pi + tau_ns)
        MW_ref_delays2.append(MW_ref_delays2[NXY8*8] + pi + tau_ns/2 + tauExtra)
        when_pulse_ref_end2 = MW_ref_delays2[NXY8*8+1] + pi_half
        #################### extra pulse for driven corr ##################
        pi_ref_delay                = np.max((when_pulse_ref_end,when_pulse_ref_end2)) + XY8_to_DC_delay
        when_pi_ref_end             = pi_ref_delay + pulse_time_ref
        pi_ref_delay2               = pi_ref_delay + shift_btwn_2NV_MW
        when_pi_ref_end2            = pi_ref_delay2 + pulse_time_ref2
        ################## Ionization ##################################################################
        ...
        pulse_to_MWmix_wait     = MWmix_delay - when_pi_end
        ...
        pulse_to_MWmix_wait2     = MWmix_NV2_delay - when_pi_end2
        ...
        if self.ifAWG:
            ...
            ch1plot, ch2plot = AWG.send_XY8DrivenCorrSCCRRIrber_seq(int(MWmix_duration), int(AWG_buffer),
                            ...
                            tau=int(tau_ns), numxy8=NXY8, mode=self.ifSinDetect, shift_MW=0,
                            ...
                            pulse_time=int(pulse_time), pulse_time_ref=int(pulse_time_ref), XY8_to_DC_delay=int(XY8_to_DC_delay))
            global ch1plot2; global ch2plot2
            ch1plot2, ch2plot2 = AWG2.send_XY8DrivenCorrSCCRRIrber_seq(int(MWmix_duration), int(AWG_buffer),
                            ...
                            tau=int(tau_ns), numxy8=NXY8, mode=self.ifSinDetect, shift_MW=shift_btwn_2NV_MW,
                            ifJustRef_CorrACorr=self.ifJustRef_CorrACorr,
                            ifAntiCorrel=self.ifAntiCorrel,
                            rest_after_first_pulse=spinInit_RR_to_pi_delay,
                            ...
                            pulse_time=int(pulse_time2), pulse_time_ref=int(pulse_time_ref2), XY8_to_DC_delay=int(XY8_to_DC_delay))
############################################## Signal ######################################################################################################################
############################################################################################################################################################################
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.XY8DrivenCorrSCCRRIrberDualNVObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
