...
class T2ERRDualNV(Instrument):

    def __init__(self, name='T2ERRDualNVObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T2ERRDualNV')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.T2ERRDualNVObject_sig2, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig2'
            )
        plot.add(data.T2ERRDualNVObject_sig, name='sig')
        plot.add(data.T2ERRDualNVObject_ref2, name='ref2')
        plot.add(data.T2ERRDualNVObject_ref, name='ref')
        ...
        #         if self.RRtrackingSettings['if_tracking'] == 1 and np.max((np.max(ref), np.max(sig))) < threshold_scanVpz: 
        #             print()
        ...
        #             self.hasTracked1 = 1
        #         if self.RRtrackingSettings2['if_tracking'] == 1 and np.max((np.max(ref2), np.max(sig2))) < threshold_scanVpz2: 
        #             print()
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T2ERRDualNVObject_sig2_set.dat'
...
class Signal2(Parameter):
    def __init__(self, name='sig2', measurementObject=None, settings=None, **kwargs):
        ...
        self.tausArray = self.settings['tausArray']
        self.T2ERRDualNVObject = measurementObject
    ...
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T2ERRDualNVObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
