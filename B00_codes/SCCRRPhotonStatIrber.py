...
class SCCRRPhotonStatIrber(Instrument):

    def __init__(self, name='SCCRRPhotonStatIrberObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        if self.ifMWDuringRead:
            ...
            if (self.SRSnum == 1) or (self.SRSnum == 2):
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='SCCRRPhotonStatIrber')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/SCCRRPhotonStatIrberObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.SCCRRPhotonStatIrberObject = measurementObject
        self.loopCounter = 0
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.SCCRRPhotonStatIrberObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
