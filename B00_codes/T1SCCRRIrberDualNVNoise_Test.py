...
from B00_codes.T1SCCRRIrberDualNVNoise import *
from qcodes_contrib_drivers.drivers.Agilent.Agilent_33522A import AG33522A
###########################################################################################e#########################
reps = 2e6
for iii in range(int(reps)):
    # T1SCCRRIrberDualNVNoise
    ifRandomized = 1;  ifPlotPulse=(reps==-1);  ifMWReadLowDutyCycle = 0; ifNeedVel = 0
    ifFancySpinInit = 0; sweepWhich = 'pi_to_ion_delay'; noise_channel = 2; ifGHzNoise=1
    ifMWDuringRead=1; ifMW2DuringRead=1; ifAWG=1; ifIQ=ifAWG; ifHiloExtra=1; ifAntiCorrel=1
    laserInit_channel=3; laserIon_channel=10; hiLoMWPwr_channel=17; ifInitVpz=0; ifInitWvl=0
    ifAG33522A = 1; AGBW = 10e6; AGfreq=AGBW; AGamp = 3.8 # beware of heating!!
    
    t0 = np.linspace(20,4820,13)+800#np.linspace(20,5020,26)+800
    t1 = np.round(np.logspace(np.log10(1.2e3),np.log10(3e6),15),-1)
    t2 = np.array([ 850.,  920.])
    tausArray = np.concatenate((t1[3:-3],t2,t0))
    ##############################################################################################################
    if True:
        # NV1
        velNum = 1; vel_current = 62.7; vel_wvl = 637.22; vel_vpz_target = -1; laserRead_channel = 5
        SRSnum  = 1; MWPower  = -6.2; pi_time  = 40; MWFreq  = 2598.19e6; MWPnoOvlap  = -5.7 #NV D1 ms-1 2598.44
        SRSnum3 = 3; MWPower3 = 6.00; pi_time3 = 40; MWFreq3 = 3161.27e6                     #NV D1 ms+1
        ...
        # NV2
        velNum2 = 2; vel_current2 = 67.1; vel_wvl2 = 636.65; vel_vpz_target2=-1; laserRead2_channel = 14
        SRSnum2 = 2; MWPower2 = -9.5; pi_time2 = 44; MWFreq2 = 2788.70e6; MWPnoOvlap2 = -9.8 #NV D2 ms-1 -9.5
        SRSnum4 = 4; MWPower4 = 0.00; pi_time4 = 40; MWFreq4 = 3037.20e6                     #NV D2 ms+1
        MWI2_channel = 12; MWQ2_channel = 13; MWswitch2_channel = 11; MWswitch4_channel = 16
        SDGnum2 = 2; AWG2_channel = 19; srate2 = 250000000; amp_MW_mix2 = 0.7
    ##############################################################################################################
    num_loops                    = int(2e4);  nSpinInit                 = 0
    ...
    pi_to_ion_delay              = 2e2;       pi_to_hilo_extra_delay    = 0.8e3
    ...
    MWFreq5                      = 2.693e9;   MWPower5                  = -99
    
    settings = {'tausArray': tausArray, 'noise_channel':noise_channel, 'ifAntiCorrel':ifAntiCorrel,'ifGHzNoise':ifGHzNoise,
                ...
                'ifAG33522A':ifAG33522A, 'AGBW':AGBW, 'AGfreq':AGfreq, 'AGamp':AGamp,
                ...
                'SDGnum2': SDGnum2, 'AWG2_channel':AWG2_channel, 'srate':srate, 'srate2':srate2,'amp_MW_mix':amp_MW_mix,'amp_MW_mix2':amp_MW_mix2,
                }
    ...
    if ifAG33522A==1:
        ...
        AG.apply(function='NOIS', freq=AGfreq, amplitude=AGamp, DCoffset=0)
    else:
        AG = AG33522A()
        AG.turn_off()
    #################################
    start = time.time()
    T1SCCRRIrberDualNVNoiseObject = T1SCCRRIrberDualNVNoise(settings=settings, ifPlotPulse=ifPlotPulse) # this is implemented as an Instrument
    T1SCCRRIrberDualNVNoiseObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    
    dataFilename = T1SCCRRIrberDualNVNoiseObject.getDataFilename()
    T1SCCRRIrberDualNVNoiseObject.close()
    ...
    # tausArray = np.delete(tausArray, np.where(tausArray == 3800))
    # tausArray = np.delete(tausArray, np.where(tausArray == 4200))

    # tausArray = np.round(np.logspace(np.log10(3e3),np.log10(3e6),15),-2) # sweep pi_to_ion_delay
    # # t1 = np.linspace(3e3,30e3,10); t2 = np.linspace(35e3,80e3,10); tausArray = np.concatenate((t1,t2))
    # # tausArray = np.linspace(20,5020,26)+800
    # t1 = np.round(np.logspace(np.log10(1.2e3),np.log10(3e6),15),-1)
    # t2 = np.array([ 820.,  850.,  920., 1020, 1650])
    # tausArray = np.concatenate((t1[0:-1],t2))