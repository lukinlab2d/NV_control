...
class DrivenCorrSCCRRIrberDualNV(Instrument):

    def __init__(self, name='DrivenCorrSCCRRIrberDualNVObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='DrivenCorrSCCRRIrberDualNV')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/DrivenCorrSCCRRIrberDualNVObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.DrivenCorrSCCRRIrberDualNVObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        pulse_time = pulse_time_ref = pi_time
        ...
        spinInit_duration_each  = spinInit_RR_duration + spinInit_RR_to_pi_delay + pulse_time2 + spinInit_pi_to_RR_delay
        ...
        pi_to_MWmix_wait2        = MWmix_NV2_delay - pi_delay2 - pulse_time2
        ...
        if self.ifAWG:
            ...
            ch1plot, ch2plot = AWG.send_DrivenCorrSCCRRPhotonStatIrber_seq(int(MWmix_duration), int(AWG_buffer),
                            int(sig_to_ref_wait), pulse_time=int(pulse_time), pulse_time_ref=int(pulse_time_ref), pi_to_MWmix_wait=int(pi_to_MWmix_wait),
                            sleepTime=1,amp_MW_mix=amp_MW_mix,shift_MW=0)
            global ch1plot2; global ch2plot2
            ch1plot2, ch2plot2 = AWG2.send_DrivenCorrSCCRRPhotonStatIrber_seq(int(MWmix_duration), int(AWG_buffer),
                            int(sig_to_ref_wait2), pulse_time=int(pulse_time2), pulse_time_ref=int(pulse_time_ref2), pi_to_MWmix_wait=int(pi_to_MWmix_wait2),
...
##########################################################################################################
        ...
        if self.ifFancySpinInit:
            for i in range(nSpinInit):
                ...
                pulse_sequence += [spc.Pulse('MWswitch3',  delayMW2,           duration=int(pulse_time2))] 
                pulse_sequence += [spc.Pulse('LaserRead2', delayRR,            duration=int(spinInit_RR_duration))] 
                pulse_sequence += [spc.Pulse('MWswitch4',  delayMW2,           duration=int(pulse_time2))]
            ...
            if self.ifAWG:
                ...
                pulse_sequence += [spc.Pulse('MWswitch',      pi_delay,            duration=int(pulse_time))] 
                pulse_sequence += [spc.Pulse('MWswitch2',     pi_delay,            duration=int(pulse_time))]
...
##############################################################################################################################
        ...
        if self.ifFancySpinInit:
            ...
            if self.ifAWG == 0:
                pulse_sequence += [spc.Pulse('MWswitch',      pi_ref_delay,         duration=int(pulse_time))]
                pulse_sequence += [spc.Pulse('MWswitch2',     pi_ref_delay,         duration=int(pulse_time))]
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.DrivenCorrSCCRRIrberDualNVObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
