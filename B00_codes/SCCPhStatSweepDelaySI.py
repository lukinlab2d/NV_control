...
class SCCPhStatSweepDelaySI(Instrument):

    def __init__(self, name='SCCPhStatSweepDelaySIObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='SCCPhStatSweepDelaySI')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/SCCPhStatSweepDelaySIObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        ...
        self.SCCPhStatSweepDelaySIObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        MWI_to_shelve_delay     = self.settings['MWI_to_shelve_delay'];    shelve_duration     = self.settings['shelve_duration']  
        shelve_to_ion_delay     = tau_ns;                                  ion_duration        = self.settings['ion_duration']
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.SCCPhStatSweepDelaySIObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
