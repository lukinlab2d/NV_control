...
class T2EAWG(Instrument):

    def __init__(self, name='T2EAWGObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T2EAWG')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.T2EAWGObject_sig, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig'
            )
        plot.add(data.T2EAWGObject_ref, name='ref')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T2EAWGObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        ...
        self.T2EAWGObject = measurementObject
        self.loopCounter = 0
