...
class CalibrateReadoutLength(Instrument):

    def __init__(self, name='CalibrateReadoutLengthObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='CalibrateReadoutLength')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.CalibrateReadoutLengthObject_sig, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig'
            )
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/CalibrateReadoutLengthObject_sig_set.dat'
...
class Signal(Parameter):
    ...
    def set_raw(self, tau_ns):
        ...
        pulse_sequence += [spc.Pulse('MW_I',     MWI_delay-MWI_to_switch_delay, duration=int(MWI_duration + 2*MWI_to_switch_delay))] # times are in ns
    ...
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.CalibrateReadoutLengthObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
