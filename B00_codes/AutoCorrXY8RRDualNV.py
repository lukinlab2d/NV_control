...
class AutoCorrXY8RRDualNV(Instrument):

    def __init__(self, name='AutoCorrXY8RRDualNVObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='AutoCorrXY8RRDualNV')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.AutoCorrXY8RRDualNVObject_sig, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig'
            )
        plot.add(data.AutoCorrXY8RRDualNVObject_sig2, name='sig2')
        plot.add(data.AutoCorrXY8RRDualNVObject_ref, name='ref')
        plot.add(data.AutoCorrXY8RRDualNVObject_ref2, name='ref2')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/AutoCorrXY8RRDualNVObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, name='sig', measurementObject=None, settings=None, **kwargs):
        ...
        self.tausArray = self.settings['tausArray']
        self.AutoCorrXY8RRDualNVObject = measurementObject
        ...
        self.ifSweepPhaseLastPiHalf = self.settings['ifSweepPhaseLastPiHalf']
        self.ifSweepPhaseLastPiHalfSig = self.settings['ifSweepPhaseLastPiHalfSig']
    ...
    def set_raw(self, tcorr):
        ...
        MW_to_read_delay        = self.settings['MW_to_read_delay'];    tau_ns              = self.settings['tau']
        ...
        tauExtra                = self.settings['tauExtra']
        if self.ifSweepPhaseLastPiHalf == 1 or self.ifSweepPhaseLastPiHalfSig == 1:
            phi_IQ              = tcorr; phi_IQ2 = phi_IQ
            tcorr               = self.settings['tcorr']
            

        MW_duration_for_AWG     = int(2*int((AWG_buffer  + pi + rest_after_first_pulse + 2*(pi_half  + last_pi_2time + NXY8*tau_ns*8 + NXY8*pi*8  + tauExtra) + tcorr)/2))
        MW_duration_for_AWG2    = int(2*int((AWG_buffer2 + pi2+ rest_after_first_pulse + 2*(pi_half2 + last_pi_2time + NXY8*tau_ns*8 + NXY8*pi2*8 + tauExtra) + tcorr)/2))
        ...
        sig_to_ref_wait             = MW_ref_delays[0] - when_pulse_end
        ...
        sig_to_ref_wait2         = MW_ref_delays2[0] - when_pulse_end2
        ...
        if self.ifAWG:
            ...
            ch1plot, ch2plot = AWG.send_AutoCorrXY8_seq(mode=self.ifSinDetect, numxy8=NXY8, pi_2time=int(pi_half), 
                                                ...
                                                tauExtra=tauExtra,tcorr=int(tcorr), sleepTime=1)
            global ch1plot2; global ch2plot2
            ch1plot2, ch2plot2 = AWG2.send_AutoCorrXY8_seq(mode=self.ifSinDetect, numxy8=NXY8, pi_2time=int(pi_half2), 
                                                ...
                                                special_I_antiCorrPulse=I_antiCorrPulse,
                                                special_Q_antiCorrPulse=Q_antiCorrPulse,
                                                tauExtra=tauExtra,tcorr=int(tcorr),sleepTime=sleepTime)
    ...
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.AutoCorrXY8RRDualNVObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
