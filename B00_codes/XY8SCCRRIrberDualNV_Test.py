...
from B00_codes.XY8SCCRRIrberDualNV import *
import time, dropbox
...
###########################################################################################e#########################
reps = 1e7
for zzz in range(int(reps)):#np.linspace(0.6,0.1,26):#
    # XY8SCCRRIrberDualNV
    ...
    ifAntiCorrel=0; ifJustRef_CorrACorr=1; ifSinDetect=1; ifAWG=1; ifHiloExtra=1 # ifSinDetect, specify phase of last pulse
    ifTestSig=1; ifRndPhaseNoise=0; ifDiffFreqExp=1; singleChannel=1
    AGBW=25e3; AGfreq=2.5e6; AGamp=0.15; AGfreq2=2083333; AGamp2=0.1 # beware of heating!!
    sweepWhich='tau'; NXY81=10; NXY82=10
    ##############################################################################################################
    if True:
        # NV1
        velNum = 1; vel_current = 62.7; vel_wvl = 637.22; vel_vpz_target = -1; laserRead_channel = 5
        SRSnum  = 1; MWPower  = -4.0; pi_time  = 40; MWFreq  = 2598.44e6; MWPnoOvlap  = -5.7 #NV D1 ms-1
        ...
        SDGnum = 1; AWG_channel = 18; srate = 250000000; amp_MW_mix = 0.85
        # NV2
        velNum2 = 2; vel_current2 = 67.1; vel_wvl2 = 636.88; vel_vpz_target2=-1; laserRead2_channel = 14
        SRSnum2 = 2; MWPower2 = -9.0; pi_time2 = 40; MWFreq2 = 2788.70e6; MWPnoOvlap2 = -9.8 #NV D2 ms-1
        ...
        SDGnum2 = 2; AWG2_channel = 19; srate2 = 250000000; amp_MW_mix2 = 0.85
    ##############################################################################################################
    # f = np.linspace(1.42e6,1.6e6,201); t = 1/(2*f)*1e9 - pi_time; t = (4*np.rint(t/4))
    # tausArray = np.unique(t); tausArray = left_circular_shift(tausArray,5)
    # offset = 9200; tausArray = np.linspace(offset,offset+800,17) #13440
    # tausArray = np.linspace(1,8,8)
    tausArray = np.linspace(216,148,18)#np.array([296, 300, 292, 304, 288, 308, 284, 312, 280, 316, 276])
    num_loops                    = int(2.5e3);  nSpinInit                 = 0
    ...
    ion_duration                 = 8e3;       ion_duration2             = 8e3
    ...
    laserRead_to_MWmix           = 0;         phi_IQ = 0.50*pi; phi_IQ2 = 0.53*pi
    tauExtra                     = 0;         phi_IQ_antiCorrPulse      = 0.5*pi
    spinInit_RR_duration         = 0;         spinInit_RR_to_pi_delay   = 20
    spinInit_pi_to_RR_delay      = 0;         tau                       = int(1/(2*AGfreq)*1e9-pi_time)
    shift_btwn_2NV_MW            = 0;         tau2                      = int(1/(2*AGfreq2)*1e9-pi_time)
    ...
    settings = {'tausArray': tausArray,
                ...
                'laserRead2_channel':     laserRead2_channel,      'NXY81':int(NXY81), 'NXY82':int(NXY82),
                'MWPower3':MWPower3, 'MWFreq3': MWFreq3, 'SRSnum3': SRSnum3, 'MWPnoOvlap':MWPnoOvlap,
                'MWPower4':MWPower4, 'MWFreq4': MWFreq4, 'SRSnum4': SRSnum4, 'MWPnoOvlap2':MWPnoOvlap2,
                ...
                'tauExtra':tauExtra, 'ifHiloExtra':ifHiloExtra,
                'ifRndPhaseNoise':ifRndPhaseNoise, 'AGBW':AGBW, 'AGfreq':AGfreq, 'AGamp':AGamp,
                'AGfreq2':AGfreq2, 'AGamp2':AGamp2,
                'ifTestSig':ifTestSig,'ifDiffFreqExp':ifDiffFreqExp,'singleChannel':singleChannel,
