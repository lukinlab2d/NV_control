...
from B00_codes.XY8SCCRRIrberDualNVSingleRead import *
import dropbox
...
for iii in range(int(1e5)):
    # XY8SCCRRIrberDualNVSingleRead
    ifRandomized=1; ifPlotPulse=(reps==1); ifMWReadLowDutyCycle=0; ifNeedVel=0; ifStartInY=0
    ifFancySpinInit = 0; normalized_style = Q_FINAL; ifAntiCorrel = 0; ifSinDetect = 1
    ifMWDuringRead = 1; ifMW2DuringRead = 1; ifJustRef_CorrACorr = 2; if_tracking = 1
    ...
    ion_duration                 = 7500;      ion_duration2             = 15000
    ion_to_read_delay            = 4e6;       DAQ_duration              = 3e6
    ...
    spinInit_pi_to_RR_delay      = 50;        NXY8                      = 5
    shift_btwn_2NV_MW            = 0;         shift_btwn_2NV_read       = 0 #must be 0
    MWmix_duration_short         = -1;        delay_between_MWmix       = -1
    ######################################### Scan params ########################################
    threshold_scanVpz            = 3;         threshold_scanVpz2        = -1
    num_loops_track              = int(4e4);   
    ...
    ######################################################
    time_sleep_after_scan        = 30;         wvl_correction             = 8e-6
    ...
    settings = {'tausArray': tausArray,
                ...
                'velNum2': velNum2, 'vel_current2': vel_current2, 'vel_wvl2': vel_wvl2, 'vel_vpz_target2': vel_vpz_target2,
                ...
                'laserRead2_channel':     laserRead2_channel,      'NXY8': int(NXY8),
    ...
    #################################
    start = time.time()
    XY8SCCRRIrberDualNVSingleReadObject = XY8SCCRRIrberDualNVSingleRead(settings=settings, ifPlotPulse=ifPlotPulse) # this is implemented as an Instrument
    XY8SCCRRIrberDualNVSingleReadObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    
    dataFilename = XY8SCCRRIrberDualNVSingleReadObject.getDataFilename()
    XY8SCCRRIrberDualNVSingleReadObject.close()
