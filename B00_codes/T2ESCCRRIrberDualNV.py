...
class T2ESCCRRIrberDualNV(Instrument):

    def __init__(self, name='T2ESCCRRIrberDualNVObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T2ESCCRRIrberDualNV')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T2ESCCRRIrberDualNVObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.T2ESCCRRIrberDualNVObject = measurementObject
        self.loopCounter = 0
...
##############################################################################################################################
        ...
        if self.ifJustRef_CorrACorr == 1:
            pulse_sequence += [spc.Pulse('MWswitch', spinInit_lastMW_ref_delay,duration=int(pi_time))]
        elif self.ifJustRef_CorrACorr == 2:
            pulse_sequence += [spc.Pulse('MWswitch2', spinInit_lastMW_ref_delay,duration=int(pi_time))]
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T2ESCCRRIrberDualNVObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
