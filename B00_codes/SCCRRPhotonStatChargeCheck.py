...
class SCCRRPhotonStatChargeCheck(Instrument):

    def __init__(self, name='SCCRRPhotonStatChargeCheckObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='SCCRRPhotonStatChargeCheck')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/SCCRRPhotonStatChargeCheckObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.SCCRRPhotonStatChargeCheckObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        if sweepWhich == 'ti':
            ...
            srs2 = self.SCCRRPhotonStatChargeCheckObject.srs2
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.SCCRRPhotonStatChargeCheckObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
