...
class RabiDEER(Instrument):

    def __init__(self, name='RabiDEERObject', settings=None, ifPlotPulse=True, ifDummy=0, **kwargs) -> None:
        ...
        settings_extra = {'clock_speed': self.clock_speed, 'LaserRead': self.LaserReadParam, 'LaserInit': self.LaserInitParam,
                          'Counter': self.CounterParam, 'AWG': self.AWGParam, 'PB_type': 'USB','AWG2': self.AWGParam2,
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='RabiDEER')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.RabiDEERObject_sig, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig'
            )
        plot.add(data.RabiDEERObject_ref, name='ref')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/RabiDEERObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.RabiDEERObject = measurementObject
        self.loopCounter = 0
        self.tausArray = self.settings['tausArray']
    ...
    def set_raw(self, tau_ns):
        ...
        ch1plot2, ch2plot2 = AWG2.send_fastRabi_seq(pulse_width=int(tau_ns), buffer=int(AWGbuffer))
    ...
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.RabiDEERObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
