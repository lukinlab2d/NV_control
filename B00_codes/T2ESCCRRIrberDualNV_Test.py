...
from B00_codes.T2ESCCRRIrberDualNV import *
import dropbox
...
for iii in range(int(1e5)):
    ...
    ifFancySpinInit = 0; normalized_style = Q_FINAL; ifAntiCorrel = 2; ifSinDetect = 0
    ifMWDuringRead = 1; ifMW2DuringRead = 1; ifJustRef_CorrACorr = 0; if_tracking = 1
    ...
    taus1 = np.linspace(80,400,41); tausArray = taus1
    ##############################################################################################################
    if True:
        # NV1
    ...
    ion_duration                 = 3600;      ion_duration2             = 3600
    ...
    ######################################### Scan params ########################################
    threshold_scanVpz            = 3;         threshold_scanVpz2        = 3.2
    ...
    #################################
    start = time.time()
    T2ESCCRRIrberDualNVObject = T2ESCCRRIrberDualNV(settings=settings, ifPlotPulse=ifPlotPulse) # this is implemented as an Instrument
    T2ESCCRRIrberDualNVObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    
    dataFilename = T2ESCCRRIrberDualNVObject.getDataFilename()
    T2ESCCRRIrberDualNVObject.close()
