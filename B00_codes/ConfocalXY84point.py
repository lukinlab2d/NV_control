...
class ConfocalXY84point(Instrument):

    def __init__(self, name='ConfocalXY84pointObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        self.galvo = DAQAnalogOutputs("ConfocalXY84pointDAQ", galvo_card_name, galvo_ao_channels)
    ...
    def runScan(self):
        ...
        data = loopy.get_data_set(name='ConfocalXY84point')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/ConfocalXY84pointObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, name='sig', settings=None, measurementObject=None, **kwargs):
        ...
        self.loopCounter = 0
        self.ConfocalXY84pointObject = measurementObject
    ...
    def set_raw(self, tau_ns):
        ...
        numxy8                  = self.settings['numxy8'];                 sweepWhich = self.settings['sweepWhich']
        ...
        ifCoSweepPhiIQ          = self.settings['ifCoSweepPhiIQ']
        if ifIncPower==1:
        ...
        if ifCoSweepPhiIQ == 1:
            ...
            phi_IQ = float(phi_0_dict[tau_ns])
            # if tau_ns<=20 and numxy8==3:
            #     phi_IQ = phi_IQ + 0.1*pi
            print('Phi_IQ = %.3f pi'% (phi_IQ/pi))
        if sweepWhich == 'phi_IQ_XY8':
            phi_IQ = tau_ns
        ...
        if np.mod(self.loopCounter,len(self.ConfocalXY84pointObject.tausArray))==0:    
        ...
        #                                           sleepTime=sleepTime)
        ch1plot, ch2plot = AWG.send_XY84point_seq_varyPhi(pitime = int(pitime),   pi_2time=int(pi2time),
                                                  tau = int(tau_ns),      numxy8=numxy8,
                                                  buffer=int(AWG_buffer), sig_to_ref_wait=int(sig_to_ref_wait),
                                                  special_I=I, special_Q=Q,
                                                  sleepTime=sleepTime)
...
class VoltageX(Parameter):
    def __init__(self, name='x', measurementObject=None, settings=None, **kwargs):
        ...
        sig = self.ConfocalXY84pointObject.sig # this is implemented as a Parameter
        ref = self.ConfocalXY84pointObject.ref # this is implemented as a Parameter
        ref2 = self.ConfocalXY84pointObject.ref2
        ref3 = self.ConfocalXY84pointObject.ref3
        sigStd = self.ConfocalXY84pointObject.sigStd
        refStd = self.ConfocalXY84pointObject.refStd
        refStd2= self.ConfocalXY84pointObject.refStd2
        refStd3= self.ConfocalXY84pointObject.refStd3

        # dummy = np.linspace(0,0.1,10)
        self.loop = Loop(
            sig.sweep(keys=self.tausArray),
            delay = 0,
            sleepTimeAfterFinishing=0).each(sig,ref,ref2, ref3, sigStd, refStd, refStd2, refStd3).then(qctask(sig.close))
    ...
    def set_raw(self,x):
        ...
        if ifRandomized==1: np.random.shuffle(self.tausArray)   
        ...
        if self.XY8DictFile is not None:
            if XY8contrast <= self.XY8contrastThres:
                ...
                ifOnFlake=1
                self.ConfocalXY84pointObject.tausArray = self.tausArray
                self.loop.sweep_values = sig.sweep(keys=self.tausArray)
            else:
                print("XY8contrast = %.4f. Off flake" % XY8contrast)
                ifOnFlake=0
                self.ConfocalXY84pointObject.tausArray = self.tausArray2
