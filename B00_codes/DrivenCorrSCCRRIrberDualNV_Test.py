...
for iii in range(int(reps)):
    ...
    ifRndPhaseNoise=0; AGBW = 100e3; AGfreq = 1506024; AGamp = 0*0.25 # beware of heating!!
    
    tausArray = np.linspace(8,80,10)         # rabi #np.linspace(32,44,4) #
    ##############################################################################################################
    if True:
        # NV1
        ...
        SDGnum = 1; AWG_channel = 18; srate = 2.5e8; amp_MW_mix = 0.7 # from 0 to 1=
        # NV2
    ...
    pi_to_ion_delay              = 1e2;       pi_to_hilo_extra_delay    = 2.8e3
    ...
    laserRead_to_MWmix           = 0;         shift_btwn_2NV_MW         = 120
    ...
    #################################
    start = time.time()
    DrivenCorrSCCRRIrberDualNVObject = DrivenCorrSCCRRIrberDualNV(settings=settings, ifPlotPulse=ifPlotPulse) # this is implemented as an Instrument
    DrivenCorrSCCRRIrberDualNVObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    
    dataFilename = DrivenCorrSCCRRIrberDualNVObject.getDataFilename()
    DrivenCorrSCCRRIrberDualNVObject.close()
    ...
    # DrivenCorrSCCRRIrberDualNV
    ifRandomized = 1;  ifPlotPulse=(reps==-1);  ifMWReadLowDutyCycle = 0; ifNeedVel = 0
    ifFancySpinInit = 0; sweepWhich = 'pi_pulse_anticorr'
    ifMWDuringRead=1; ifMW2DuringRead=1; ifAWG=1; ifIQ=ifAWG; ifHiloExtra=1
    laserInit_channel=3; laserIon_channel=10; hiLoMWPwr_channel=17; ifInitVpz=0; ifInitWvl=0
    ifRndPhaseNoise=0; AGBW = 100e3; AGfreq = 1506024; AGamp = 0*0.25 # beware of heating!!
