...
class T1RRDualNV_6point(Instrument):

    def __init__(self, name='T1RRDualNV_6pointObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T1RRDualNV_6point')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T1RRDualNV_6pointObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.T1RRDualNV_6pointObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        when_DAQ_ref_NV2_end     = DAQ_ref_NV2_delay + DAQ_ref_duration
        when_laser_read_ref_NV2_end = when_DAQ_ref_NV2_end + DAQ_to_laser_off_delay

        when_everything_ref_end = np.max((when_laser_read_ref_NV2_end, when_DAQ_ref_NV2_end))
        ####### noise during ref #######
        ...
        shift_btwn_exp = laser_init_ref_delay - laser_init_delay
...
############## 4 other experiments ###########################
        for i in np.linspace(1,4,4):
            laser_init_sigM_delay = int(laser_init_ref_delay + i*shift_btwn_exp)
            laser_read_sigM_delay = int(laser_read_ref_delay + i*shift_btwn_exp)
            DAQ_sigM_delay        = int(DAQ_ref_delay + i*shift_btwn_exp)
            laser_read_sigM_NV2_delay = int(laser_read_ref_NV2_delay + i*shift_btwn_exp)
            DAQ_sigM_NV2_delay    = int(DAQ_ref_NV2_delay + i*shift_btwn_exp)

            if not laser_init_delay == 0:
                pulse_sequence += [spc.Pulse('LaserInit',laser_init_sigM_delay, duration=int(laser_init_duration))]
                # Read ref NV1
                pulse_sequence += [spc.Pulse('LaserRead',laser_read_sigM_delay, duration=int(laser_read_ref_duration))] 
                pulse_sequence += [spc.Pulse('Counter',  DAQ_sigM_delay,        duration=int(DAQ_ref_duration))] 

                # Read ref NV2
                pulse_sequence += [spc.Pulse('LaserRead2',   laser_read_sigM_NV2_delay, duration=int(laser_read_signal_duration2))] 
                pulse_sequence += [spc.Pulse('Counter',      DAQ_sigM_NV2_delay,        duration=int(DAQ_signal_duration2))]  
        
        ################# Add pi pulses accordingly for the ms=+1 pulses #################
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T1RRDualNV_6pointObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
