...
from B00_codes.XY8DrivenCorrSCCRRIrberDualNV import *
import dropbox
...
for iii in range(int(reps)):
    ...
    ifAntiCorrel=0; ifJustRef_CorrACorr=0; ifSinDetect=0; ifAWG=1; ifHiloExtra=1 # ifSinDetect, specify phase of last pulse
    ifTestSig=0; ifRndPhaseNoise=0; AGBW = 100e3; AGfreq = 1506024; AGamp = 0.25 # beware of heating!!
    sweepWhich='pi_pulse'; NXY8=5
    ##############################################################################################################
    if True:
        # NV1
        ...
        SDGnum = 1; AWG_channel = 18; srate = 250000000; amp_MW_mix = 0.95
        # NV2
        ...
        SDGnum2 = 2; AWG2_channel = 19; srate2 = 250000000; amp_MW_mix2 = 0.8
    ##############################################################################################################
    # f = np.linspace(1.48e6,1.64e6,201)
    # t = 1/(2*f)*1e9 - pi_time; t = (4*np.rint(t/4)); tausArray = np.unique(t)
    # offset = 0; tausArray = np.linspace(offset,offset+1920,7) #13440
    tausArray = np.linspace(8,80,10)

    num_loops                    = int(5e3); nSpinInit                 = 0
    ...
    settings = {'tausArray': tausArray,
                ...
                'tau':tau, 'sweepWhich':sweepWhich,'amp_MW_mix':amp_MW_mix,'amp_MW_mix2':amp_MW_mix2,'XY8_to_DC_delay':XY8_to_DC_delay}
    ####### Random-phase noise ######
    if ifTestSig==1:
        AG = AG33522A()
        AG.disable_PM()
    ...
    #################################
    start = time.time()
    XY8DrivenCorrSCCRRIrberDualNVObject = XY8DrivenCorrSCCRRIrberDualNV(settings=settings, ifPlotPulse=ifPlotPulse) # this is implemented as an Instrument
    XY8DrivenCorrSCCRRIrberDualNVObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    XY8DrivenCorrSCCRRIrberDualNVObject.close()
