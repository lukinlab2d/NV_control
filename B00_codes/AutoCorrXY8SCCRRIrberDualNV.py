...
class AutoCorrXY8SCCRRIrberDualNV(Instrument):

    def __init__(self, name='AutoCorrXY8SCCRRIrberDualNVObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='AutoCorrXY8SCCRRIrberDualNV')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/AutoCorrXY8SCCRRIrberDualNVObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.AutoCorrXY8SCCRRIrberDualNVObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tcorr):
        ...
        pi = pi_time; pi_half=pi/2;                                       tau_ns = self.settings['tau']

        if sweepWhich=='shift_btwn_2NV_MW':
            shift_btwn_2NV_MW = tcorr
            tcorr = self.settings['tcorr']
        ...
        when_pulse_corr_end = when_pulse_end + tcorr + (when_pulse_end-MWI_delay)
        ###### NV2 ######################################
        ...
        when_pulse_corr_end2 = when_pulse_end2 + tcorr + (when_pulse_end2-MWI_delay2)
        ################## Ionization ###################################################################
        ion_RR_delay            = np.max((when_pulse_corr_end,when_pulse_corr_end2)) + pi_to_ion_delay               
        ...
        when_pulse_corr_ref_end = when_pulse_ref_end + tcorr + (when_pulse_ref_end-MWI_ref_delay)
        ...
        when_pulse_corr_ref_end2 = when_pulse_ref_end2 + tcorr + (when_pulse_ref_end2-MWI_ref_delay2)
        ################## Ionization ##################################################################
        ion_ref_RR_delay        = np.max((when_pulse_corr_ref_end, when_pulse_corr_ref_end2)) + pi_to_ion_delay
        ...
        pulse_to_MWmix_wait     = MWmix_delay - when_pulse_corr_end
        ...
        pulse_to_MWmix_wait2     = MWmix_NV2_delay - when_pulse_corr_end2
        ...
        if self.ifAWG:
            ...
            ch1plot, ch2plot = AWG.send_AutoCorrXY8SCCRRIrber_seq(int(MWmix_duration), int(AWG_buffer),
                            ...
                            tauExtra=tauExtra,amp_MW_mix=amp_MW_mix,tcorr=int(tcorr))
            global ch1plot2; global ch2plot2
            ch1plot2, ch2plot2 = AWG2.send_AutoCorrXY8SCCRRIrber_seq(int(MWmix_duration), int(AWG_buffer),
                            ...
                            special_I_antiCorrPulse=I_antiCorrPulse,
                            special_Q_antiCorrPulse=Q_antiCorrPulse,
                            tauExtra=tauExtra,amp_MW_mix=amp_MW_mix2,tcorr=int(tcorr))
############################################## Signal ######################################################################################################################
############################################################################################################################################################################
...
##################################################################################################################################################################################
        ...
        if sweepWhich=='shift_btwn_2NV_MW':
            ...
            print('Set tcorr to ' + str(tcorr) + " ns")
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.AutoCorrXY8SCCRRIrberDualNVObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
