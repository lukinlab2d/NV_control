...
class CalibYellowIonizeRate2(Instrument):

    def __init__(self, name='CalibYellowIonizeRate2Object', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='CalibYellowIonizeRate2')
        data.add_metadata(self.settings)
        self.data = data
        
        # plot = QtPlot(
        #     data.CalibYellowIonizeRate2Object_sig, # this is implemented as a Parameter
        #     figsize = (1200, 600),
        #     interval = 1,
        #     name = 'sig'
        #     )
        # plot.add(data.CalibYellowIonizeRate2Object_ref, name='ref')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/CalibYellowIonizeRate2Object_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        ...
        self.CalibYellowIonizeRate2Object = measurementObject
        self.loopCounter = 0
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.CalibYellowIonizeRate2Object.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
