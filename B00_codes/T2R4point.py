...
class T2R4point(Instrument):

    def __init__(self, name='T2R4pointObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T2R4point')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.T2R4pointObject_sig, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig'
            )
        plot.add(data.T2R4pointObject_ref, name='ref')
        plot.add(data.T2R4pointObject_ref2, name='ref2')
        plot.add(data.T2R4pointObject_ref3, name='ref3')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T2R4pointObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        ...
        self.T2R4pointObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        ch1plot, ch2plot = AWG.send_T2R4point_seq(pi_2time=int(pi2time), tau = int(tau_ns), buffer=int(AWG_buffer), sig_to_ref_wait=int(sig_to_ref_wait))
    ...
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T2R4pointObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
