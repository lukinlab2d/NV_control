...
####################################################################################################################
# T1
reps = int(111); ifLooped = (reps!=-1); srate=None #2.5e8, just a pi pulse. No need sampling mode
for i in np.linspace(1,reps,reps):
    ...
    ifRandomized = 0; ifPlotPulse=0
    tausArray = np.round(np.logspace(np.log10(40),np.log10(12e6),16),-1)
    uwPower = -8; uwFreq = 2680.02e6
    # Too few loops like 3e4 is bad??
    num_loops               = int(1e5); ifShimon                = 0  # do sig-ref under the same green
    laser_init_delay        = 0;        laser_init_duration     = 0
    laser_to_AWG_delay      = 0;        pi_time                 = 31
    laser_to_DAQ_delay_directory = {3: 850, 6: 1150, 9: 1150, 7: 900}
    laser_to_DAQ_delay = laser_to_DAQ_delay_directory.get(laserRead_channel, 0)
    read_duration           = 300;      DAQ_to_laser_off_delay  = 6000 
    sig_to_ref_delay_Shimon = 1000;     MW_to_read_delay        = 100
    ...
    if True:
        ...
        settings = {'tausArray': tausArray,
                    ...
                    'srate':srate,'MW_to_read_delay':MW_to_read_delay
                    }
