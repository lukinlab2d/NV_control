...
class T1SCCRRIrberDualNVNoise(Instrument):

    def __init__(self, name='T1SCCRRIrberDualNVNoiseObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T1SCCRRIrberDualNVNoise')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T1SCCRRIrberDualNVNoiseObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.T1SCCRRIrberDualNVNoiseObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        ifAntiCorrel            = self.settings['ifAntiCorrel'];           ifGHzNoise             = self.settings['ifGHzNoise']
        ...
        when_pi_end2            = pi_delay2 + pi_time2
        ...
        hilo_extra_delay           = when_pi_end + pi_to_hilo_extra_delay ## why not when_pi_end_2???
        ...
        when_pi_ref_end2        = pi_ref_delay2 + pi_time2
        ...
        if self.loopCounter==0 or tau_ns>=15e6: sleepTime = 15
        else: sleepTime = 6
        if tau_ns>=30e6: sleepTime = 20
        if self.ifAWG:
            global ch1plot; global ch2plot
            ch1plot, ch2plot = AWG.send_T1SCCRRnoise_seq(int(MWmix_duration), int(AWG_buffer),
                            int(sig_to_ref_wait), pitime=int(pi_time), pi_to_MWmix_wait=int(pi_to_MWmix_wait),
                            sleepTime=1,amp_MW_mix=amp_MW_mix,ifAntiCorrel=ifAntiCorrel,AWGnum=1)
            global ch1plot2; global ch2plot2
            ch1plot2, ch2plot2 = AWG2.send_T1SCCRRnoise_seq(int(MWmix_duration), int(AWG_buffer),
                            int(sig_to_ref_wait2), pitime=int(pi_time2), pi_to_MWmix_wait=int(pi_to_MWmix_wait2),
                            sleepTime=sleepTime,amp_MW_mix=amp_MW_mix2,ifAntiCorrel=ifAntiCorrel,AWGnum=2)
##########################################################################################################
##########################################################################################################
        ...
        # Ionization
        if ifGHzNoise==1 and ifAntiCorrel!=0:
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T1SCCRRIrberDualNVNoiseObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
