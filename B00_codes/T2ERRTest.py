...
pi = np.pi
####################################################################################################################
reps = 500;  ifRandomized = 1; ifLooped = (reps!=1); normalized_style = Q_FINAL
ifAWG=1;   laserInit_channel = 3;      ifScanVpz = 0;    
vel_vpz_start     = 57;  vel_vpz_end       = 62
vel_vpz_step      = 0.1; vel_vpz_step_time = 0.3; 

# T2ERR   
tausArray = np.round(np.logspace(2,np.log10(2e6),61),-1)
# tausArray = np.linspace(20,9920,100); t2 = np.linspace(10000,100000,91)
# tausArray = np.concatenate((tausArray,t2))
...
if True: 
    SRSnum = 1; MWPower = 0; pi_half = 34; MWFreq  = 1930e6   #NV D1
...
laser_init_delay             = 5e2;        laser_init_duration    = 6e3
...
for i in np.linspace(1,reps,reps):
    ...
    settings = {'tausArray': tausArray, 'num_loops':num_loops, 'MWPower':MWPower, 'MWFreq': MWFreq, 'SRSnum': SRSnum,
                ...
                'ifAWG': ifAWG, 'SDGnum': SDGnum, 'AWG_channel':AWG_channel, 'srate':srate,'AWG_buffer':AWG_buffer, 'AWG_output_delay':AWG_output_delay  }
