...
class ConfocalXY84pointDrivenSDG(Instrument):

    def __init__(self, name='ConfocalXY84pointDrivenObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        self.add_parameter(
            name = "refStd2",
            parameter_class = RefStd2,
        )
        self.add_parameter(
            name = "refStd3",
            parameter_class = RefStd3,
        )
        self.add_parameter(
            name = "y",
            settings = self.settings,
        ...
        self.galvo = DAQAnalogOutputs("ConfocalXY84pointDrivenDAQ", galvo_card_name, galvo_ao_channels)
    ...
    def runScan(self):
        ...
        data = loopy.get_data_set(name='ConfocalXY84pointDriven')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/ConfocalXY84pointDrivenObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, name='sig', settings=None, measurementObject=None, **kwargs):
        ...
        self.loopCounter = 0
        self.ConfocalXY84pointDrivenObject = measurementObject
    ...
    def get_raw(self):      
        ...
        ref3 = rate[3::self.num_reads_per_iter]
        ...
        global ref_std2; ref_std2= np.std(ref2)
        global ref_std3; ref_std3= np.std(ref3)
    ...
    def set_raw(self, drive_freq):
        ...
        AWG_buffer              = self.settings['AWG_buffer'];             phi_IQ_XY8          = self.settings['phi_IQ_XY8']
        sweepWhich              = self.settings['sweepWhich'];             phi_IQ_RF           = self.settings['phi_IQ_RF']
        ifCoSweepPhiIQXY8       = self.settings['ifCoSweepPhiIQXY8']
        if sweepWhich=='phi_IQ_RF':
            phi_IQ_RF = drive_freq
        ...
        if ifCoSweepPhiIQXY8==1:
            phi_0_dict = self.settings['phi_0_dict']
            phi_IQ_XY8 = float(phi_0_dict[tau_ns])
            # if tau_ns<=20 and numxy8==1:
            #     phi_IQ = 0.63*pi
            print('Phi_IQ_XY8 = %.3f pi'% (phi_IQ_XY8/pi))
        ...
        if np.mod(self.loopCounter,len(self.ConfocalXY84pointDrivenObject.tausArray))==0:    
        ...
        hilo_delay2     = when_laser_read_signal_end + hilo_delay
        hilo_delay3     = when_laser_read_signal_end + hilo_delay2
        ...
        MW_del_ref2      = when_laser_read_ref_end + MW_del
        AWG_trig_delay3  = MW_del_ref2 - AWG_output_delay
        AWG2_trig_delay3 = AWG_trig_delay3 - 100
        ...
        when_read_ref_end2    = read_ref_delay2 + read_ref_duration
        when_laser_read_ref_end2 = laser_read_ref_delay2 + laser_read_ref_duration

        MW_del_ref3      = when_laser_read_ref_end2 + MW_del
        AWG_trig_delay4  = MW_del_ref3 - AWG_output_delay
        AWG2_trig_delay4 = AWG_trig_delay4 - 100
        ...
        when_read_ref_end3    = read_ref_delay3 + read_ref_duration
        when_laser_read_ref_end3 = laser_read_ref_delay3 + laser_read_ref_duration
        ...
        I = round(32767*np.cos(phi_IQ_XY8))
        Q = round(32767*np.sin(phi_IQ_XY8))
        ...
        ch1plot, ch2plot = AWG.send_XY84point_seq(pitime = int(pitime),   pi_2time=int(pi2time),
                                                  tau = int(tau_ns),      numxy8=numxy8,
        ...
        AWG2.upload_waveform_generator(freq=drive_freq, Vpp=MWPower2, 
                                       phase=np.rad2deg(phi_IQ_RF), Ncycle=Ncycle)
        ...
        pulse_sequence += [spc.Pulse('LaserRead',    laser_read_ref_delay2,         duration=int(laser_read_ref_duration))]
        pulse_sequence += [spc.Pulse('LaserRead',    laser_read_ref_delay3,         duration=int(laser_read_ref_duration))]
        ...
        pulse_sequence += [spc.Pulse('AWG2',         AWG2_trig_delay3,              duration=50)]
        pulse_sequence += [spc.Pulse('AWG2',         AWG2_trig_delay4,              duration=50)]
        ...
        pulse_sequence += [spc.Pulse('hiLoMWPwr',    hilo_delay2,               duration=int(hilo_duration))]
        pulse_sequence += [spc.Pulse('hiLoMWPwr',    hilo_delay3,               duration=int(hilo_duration))]
        pulse_sequence += [spc.Pulse('hiLoMWPwr',    laser_read_ref_delay3,     duration=int(laser_read_ref_duration))]
        ...
        pulse_sequence += [spc.Pulse('Counter',      read_ref_delay2,               duration=int(read_ref_duration))] # times are in ns
        pulse_sequence += [spc.Pulse('Counter',      read_ref_delay3,               duration=int(read_ref_duration))] # times are in ns
        ...
        elif sweepWhich=='phi_IQ_RF':
            print('Set drive phase to ' + str(phi_IQ_RF/np.pi) + " pi")
        elif sweepWhich=='MWPower2':
            print('Set drive freq to ' + str(drive_freq/1e6) + " MHz")
            print('Set drive phase to ' + str(phi_IQ_RF/np.pi) + " pi")
    ...
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.ConfocalXY84pointDrivenObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
...
class RefStd2(Parameter):
    def __init__(self, name='refStd2',**kwargs):
        super().__init__(name, **kwargs)

    def get_raw(self):
        return ref_std2

class RefStd3(Parameter):
    def __init__(self, name='refStd3',**kwargs):
        super().__init__(name, **kwargs)
...
class VoltageX(Parameter):
    def __init__(self, name='x', measurementObject=None, settings=None, **kwargs):
        ...
        sig = self.ConfocalXY84pointDrivenObject.sig # this is implemented as a Parameter
        ref = self.ConfocalXY84pointDrivenObject.ref # this is implemented as a Parameter
        ref2 = self.ConfocalXY84pointDrivenObject.ref2
        ref3 = self.ConfocalXY84pointDrivenObject.ref3
        sigStd = self.ConfocalXY84pointDrivenObject.sigStd
        refStd = self.ConfocalXY84pointDrivenObject.refStd
        refStd2 = self.ConfocalXY84pointDrivenObject.refStd2
        refStd3 = self.ConfocalXY84pointDrivenObject.refStd3

        self.loop = Loop(
            sig.sweep(keys=self.tausArray),
            delay = 0,
            sleepTimeAfterFinishing=0).each(sig,ref,ref2, ref3, sigStd, refStd, refStd2, refStd3,
    ...
    def set_raw(self,x):
        ...
        if self.XY8DictFile is not None:
            if XY8contrast <= self.XY8contrastThres:
                ...
                ifOnFlake=1
                self.ConfocalXY84pointDrivenObject.tausArray = self.tausArray
                self.loop.sweep_values = sig.sweep(keys=self.tausArray)
            else:
                print("XY8contrast = %.4f. Off flake" % XY8contrast)
                ifOnFlake=0
                self.ConfocalXY84pointDrivenObject.tausArray = self.tausArray2
