...
for ii in range(10000):
    ...
    uwPower  = 0; uwFreq  = 2614e6
    uwPower2 = -2; pitimeDEER =  88#; uwFreq2 = 3062.65e6
    freqs = np.linspace(2807e6,2817e6,reps)
    for i in range(reps):
        uwFreq2 = freqs[i]; print(uwFreq2)
        tausArray = np.linspace(60,6060,31)

        # Params for T2EDEERNeumannEcho
        laserInit_channel            = 3;        laserRead_channel   = 3
        num_loops                    = int(1e5); phi_IQ              = pi/2 #rad, angle of last pi/2
        laser_init_delay             = 0;        laser_init_duration = 0
        pi2time                      = 44;       pitime              = 88
        laser_to_DAQ_delay_directory = {3: 850, 6: 1150, 9: 1150, 7: 900}
        laser_to_DAQ_delay           = laser_to_DAQ_delay_directory.get(laserRead_channel, 0) 
        laser_to_AWG_delay           = 1e3;      read_duration       = 300
        DAQ_to_laser_off_delay       = 3e3;      MW_to_read_delay    = 40
        AWG_output_delay             = 1450;     tau_echo            = 6500
        ...
        start = time.time()
        T2EDEERNeumannEchoObject = T2EDEERNeumannEcho(settings=settings, ifPlotPulse=not(ifLooped)) # this is implemented as an Instrument
        T2EDEERNeumannEchoObject.runScan()
        print('Total time = ' + str(time.time() - start) + ' s')
        T2EDEERNeumannEchoObject.close()