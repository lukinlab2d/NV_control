...
from B00_codes.ScanRRFreqSCC import *
...
for whichNV in np.linspace(1,2,1):
    ...
    ifRandomized = 0; laserInit_channel = 3; laserIon_channel = 10; hiLoMWPwr_channel = 17; 
    ifRndPhaseNoise = 1; AGBW = 1000e3; AGfreq = 1506024; AGamp = 0.25 # beware of heating!!
    if np.mod(whichNV,2)==1: 
        ...
        MWI2_channel = 0; MWQ2_channel = 0; MWswitch2_channel = 15; amp_MW_mix = 0.8
        start = 13.6; stop = 14.2; num_sweep_points = int(31) # slow scan
    else:
        ...
        MWI2_channel = 0; MWQ2_channel = 0; MWswitch2_channel = 16; amp_MW_mix = 0.8
        start = 40.8; stop = 41.4; num_sweep_points = int(31) # slow scan
    vpzArray = np.linspace(start, stop, num_sweep_points)

    num_loops                    = int(1e3); nSpinInit                 = 0
    ...
    pi_to_ion_delay              = 5e2;        ion_duration              = 4e3
    ...
    settings = {'vpzArray': vpzArray, 'dbx':dbx,
                ...
                'ifAWG': ifAWG, 'ifIQ': ifIQ,'amp_MW_mix':amp_MW_mix,
    ...
    #################################
    start = time.time()
    ScanRRFreqSCCObject = ScanRRFreqSCC(settings=settings, ifPlotPulse=ifPlotPulse) # this is implemented as an Instrument
    ScanRRFreqSCCObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    ScanRRFreqSCCObject.close()
