...
class PLSCCRRDualNV(Instrument):

    def __init__(self, name='PLSCCRRDualNVObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='PLSCCRRDualNV')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/PLSCCRRDualNVObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.PLSCCRRDualNVObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        pi_delay                = when_firstPiHalf_end
        when_pi_end             = pi_delay + pi_time
        secondPiHalf_delay      = when_pi_end
        ...
        pi_ref_delay                 = when_firstPiHalf_ref_end
        when_pi_ref_end              = pi_ref_delay + pi_time
        secondPiHalf_ref_delay       = when_pi_ref_end
...
##############################################################################################################
        ...
        pulse_sequence += [spc.Pulse('LaserRead2',    ion_RR2_delay,           duration=int(ion_duration))] 
...
##############################################################################################################################
        ...
        pulse_sequence += [spc.Pulse('LaserRead2',   ion_ref_RR2_delay,    duration=int(ion_duration))]
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.PLSCCRRDualNVObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
