...
reps=int(1); ifLooped=(reps!=1); laserInit_channel=3; normalized_style=Q_FINAL; hiLoMWPwr_channel=17 
ifInitWvl=0; ifInitVpz=0; ifNeedVel1=0; ifNeedVel2=0; if_tracking=0; ifRandomized=0; ifStartInY=0
ifHiloExtra=1; ifAntiCorrel=0; ifJustRef_CorrACorr=0; ifSinDetect=0; ifAWG=1; sweepWhich='tau'#'phase_last_pi_half' # ifSinDetect, specify phase of last pulse
ifTestSig=1; ifRndPhaseNoise=0; ifDiffFreqExp=1; singleChannel=1
AGBW=25e3; AGfreq=2.5e6; AGamp=0.15; AGfreq2=2083333; AGamp2=0.3
AGampmax=0.8; AGampArr=np.linspace(0,AGampmax,26)
for i in np.linspace(1,reps,reps):#AGamp in AGampArr:
    print('AGamp: ', AGamp)
    # tausArray = np.linspace(160,160,1) #tausArray = np.linspace(0*pi, 2.4*pi, 31) #
    # tausArray = np.round(np.logspace(np.log10(3e3),np.log10(1e6),31)/5/8,-1); tausArray = (4*np.rint(tausArray/4))
    # tausArray = np.array([296,])# 300, 292, 304, 288, 308, 284, 312, 280, 316, 276])-4
    tausArray = np.linspace(140,224,22) #np.linspace(1,20,20)
    # f = np.linspace(1.3e6,3.4e6,201)
    # t = 1/(2*f)*1e9 - 40; t = (4*np.rint(t/4)); tausArray = np.unique(t)

    num_loops                    = int(5e5);   NXY81 = int(10); NXY82 = int(10)
    laser_init_delay             = 5e2;        laser_init_duration    = 30e3
    MW_to_read_delay             = 1e2;        MWI_to_switch_delay    = 30
    laser_to_DAQ_delay_directory = {3: 860, 6: 1160, 9: 1160, 7: 900, 5: 1660, 14:900}
    laser_to_MWI_delay           = laser_to_DAQ_delay_directory.get(laserInit_channel, 0) + 160
    ...
    AWG_buffer                   = 20;         AWG_output_delay       = 1440 
    AWG_buffer2                  = 20;         AWG_output_delay2      = 1440
    phi_IQ = 0.50*pi; phi_IQ2    = 0.53*pi;    tau                    = int(1/(2*AGfreq)*1e9-40)
    ...
    if True:
        ...
        SRSnum = 1; MWPower     = -4.0; pi_half = 20; pi_pulse = 40;  MWFreq = 2598.44e6   #NV D1 ms-1
        ...
        SRSnum2 = 2; MWPower2   = -8.3; pi_half2 = 20; pi_pulse2 = 40; MWFreq2= 2788.70e6   #NV D2 ms-1
    ...
    settings = {'tausArray': tausArray, 'num_loops':num_loops, 'MWPower':MWPower, 'MWFreq': MWFreq, 'SRSnum': SRSnum,
                ...
                'pi_pulse':pi_pulse, 'pi_pulse2':pi_pulse2,
                ...
                'ifStartInY':ifStartInY, 'NXY81': int(NXY81), 'NXY82': int(NXY82),
                'ifSinDetect':ifSinDetect, 'ifAWG':ifAWG,
                ...
                'ifRndPhaseNoise':ifRndPhaseNoise,'ifTestSig':ifTestSig, 'ifDiffFreqExp':ifDiffFreqExp,
                'AGBW':AGBW,'AGfreq':AGfreq,'AGamp':AGamp,'singleChannel':singleChannel,'AGfreq2':AGfreq2, 'AGamp2':AGamp2,
                'phi_IQ':phi_IQ, 'tau':tau, 'phi_IQ2':phi_IQ2, 'srate':srate,
                ...
                'AGampArr':AGampArr,'ifHiloExtra':ifHiloExtra,
                'hilo_margin_start':hilo_margin_start, 'hilo_margin_end':hilo_margin_end,'hilo_min':hilo_min}

    ####### Random-phase noise ######
    AG = AG33522A()
    AG.doExperiment(ifTestSig, ifRndPhaseNoise, ifDiffFreqExp, singleChannel=singleChannel, 
                    AGBW=AGBW, AGfreq=AGfreq, AGamp=AGamp, AGfreq2=AGfreq2, AGamp2=AGamp2)
    #################################
    start = time.time()
    ...
    # t = 1/(2*f)*1e9 - 40; t = (4*np.rint(t/4)); tausArray = np.unique(t) #1.506024e6
