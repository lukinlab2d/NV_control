...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        CounterParam = {'delay_time': 2, 'channel':4}
        AFGParam = {'delay_time': 2, 'channel':1}
        MWswitchParam = {'delay_time': 2, 'channel':2}
        
        settings = {'clock_speed': clock_speed, 'Laser': LaserParam, 'Counter': CounterParam, 'AFG': AFGParam, 
                    'MWswitch': MWswitchParam,'PB_type': 'USB', 'min_pulse_dur': int(5*1e3/clock_speed)}

        pb = spc.B00PulseBlaster("SpinCorePB", settings=settings, verbose=False)
        # print("Is pb an Instrument? " + str(isinstance(pb, Instrument)))

        num_loops = int(3e1); samp_rate = 1e7
        laser_delay_in_ns  = 1e3; laser_duration_in_ns   = 4e6-1e3+1e6
        read_delay_in_ns   = 4e6; read_duration_in_ns    = 1e6
        read_delay_in_ns2  = 5.1e6; read_duration_in_ns2 = 1e6
        AFG_delay_in_ns    = 200e3; AFG_duration_in_ns   = 1e6
        ...
        pulse_sequence += [spc.Pulse('Counter', read_delay_in_ns2,  duration=int(read_duration_in_ns2))] # times are in ns
