...
for iii in range(int(reps)):
    ...
    ifFancySpinInit = 0; sweepWhich = 'ti'; ifIznNV2later=0
    ...
    # tausArray[-1] = 25e6;tausArray = np.flip(tausArray)
    # tausArray = np.array((5e2,1e3,2.5e3,4e3,6e3,8e3,10e3,12.5e3,16e3,20e3,25e3)) # ti
    tausArray = np.array((5e2,1e3,2e3,3e3,4e3,5e3,6e3,7e3,8e3,10e3,12.5e3,15e3,17.5e3,20e3,25e3)) # ti
    # tausArray = np.array((7.5e5,1e6,1.2e6,1.5e6,2e6))#,3e6))#,4e6))#,5e6,6.5e6,8e6))#,10e6,12e6)) # tinit
    ##############################################################################################################
    if True:
         # NV1
