...
from B00_codes.ConfocalRRSingleRead import *
...
for i in np.linspace(1, reps, reps):
    ...
    start = time.time()
    ConfocalRRSingleReadObject = ConfocalRRSingleRead(settings=settings, ifPlotPulse=not(ifLooped)) 
    ConfocalRRSingleReadObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    ConfocalRRSingleReadObject.close()
