...
for iii in range(int(reps)):
    ...
    tausArray = tausArray[:-3]; tausArray = np.flip(tausArray)
    ##############################################################################################################
    if True:
        # NV1
        velNum = 1; vel_current = 62.7; vel_wvl = 637.22; vel_vpz_target = -1; laserRead_channel = 5
        SRSnum  = 1; MWPower  = -5.7; pi_time  = 40; MWFreq  = 2598.44e6; MWPnoOvlap  = -5.7 #NV D1 ms-1
        SRSnum3 = 3; MWPower3 = 5.50; pi_time3 = 40; MWFreq3 = 3161.27e6                     #NV D1 ms+1
        ...
        SRSnum2 = 2; MWPower2 = -9.8; pi_time2 = 40; MWFreq2 = 2788.70e6; MWPnoOvlap2 = -9.8 #NV D2 ms-1
        SRSnum4 = 4; MWPower4 = 1.00; pi_time4 = 40; MWFreq4 = 3037.20e6                     #NV D2 ms+1
    ...
    ##############################################################################################################
    num_loops                    = int(12.5e3);  nSpinInit                 = 0
    ...
    pi_to_ion_delay              = 5e2;       pi_to_hilo_extra_delay    = 2.8e3
    ...
    dutycycle_noise = np.random.uniform(0.1, 0.4); amp_noise = 0.2; ifTek = 1
    print(dutycycle_noise)
    settings = {'tausArray': tausArray, 'noise_channel':noise_channel,'ifTek':ifTek,
                ...
                'dutycycle_noise':dutycycle_noise,'amp_noise':amp_noise,
    ...
    #################################
    start = time.time()
    T1SCCRRIrberDualNVNoiseAWGObject = T1SCCRRIrberDualNVNoiseAWG(settings=settings, ifPlotPulse=ifPlotPulse) # this is implemented as an Instrument
    T1SCCRRIrberDualNVNoiseAWGObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    
    dataFilename = T1SCCRRIrberDualNVNoiseAWGObject.getDataFilename()
    T1SCCRRIrberDualNVNoiseAWGObject.close()
