...
def random_array(N, m1):
    # Step 1: Initialize array with 0's
    arr = [0] * N
    
    # Step 2: Randomly select m unique indices
    ones_indices = random.sample(range(N), m1)  # Randomly select m indices
    
    # Step 3: Set the elements at those indices to 1
    for index in ones_indices:
        arr[index] = 1
    arr = np.array(arr, dtype=np.int16)
    return arr

class T1SCCRRIrberDualNVNoiseAWG(Instrument):

    def __init__(self, name='T1SCCRRIrberDualNVNoiseAWGObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        self.noiseParam    =    {'delay_time': 2, 'channel':settings['noise_channel']}
        ...
        settings_extra = {'clock_speed': self.clock_speed, 'Counter': self.CounterParam, 
                          'AWG': self.AWGParam, 'AWG2': self.AWG2Param, 'noise': self.noiseParam,
        ...
        MWPower5 = self.settings['MWPower5']; MWFreq5 = self.settings['MWFreq5']
        ...
        if True:
            ...
            self.srs5 = SRS(SRSnum=5)
            self.srs5.set_freq(MWFreq5) #Hz
            self.srs5.set_RFAmplitude(MWPower5) #dBm
            self.srs5.enableIQmodulation()
            time.sleep(1)
            self.srs5.enable_RFOutput()
            global srs5; srs5 = self.srs5
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T1SCCRRIrberDualNVNoiseAWG')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T1SCCRRIrberDualNVNoiseAWGObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.T1SCCRRIrberDualNVNoiseAWGObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        pi_pulse_bf_read        = self.settings['pi_pulse_bf_read'];       pi_time3               = self.settings['pi_time3']
        dutycycle_noise         = self.settings['dutycycle_noise'];        amp_noise              = self.settings['amp_noise']
        ifTek                   = self.settings['ifTek']
        ...
        self.delay_for_plot     = pi_delay
        when_pi_end             = pi_delay + pi_time

        pi_delay2               = pi_delay + shift_btwn_2NV_MW
        self.delay_for_plot2    = pi_delay2
        when_pi_end2            = pi_delay2 + pi_time

        ion_RR_delay            = when_pi_end2 + pi_to_ion_delay                 
        ...
        pi_bf_read_sig_delay    = ion_RR_delay - 2*pi_time3 - shift_btwn_2NV_MW
        pi_bf_read_sig_delay2   = pi_bf_read_sig_delay + shift_btwn_2NV_MW
        ...
        rest_btwn_pi_bf_readsig_and_MWmix  = MWmix_delay - pi_bf_read_sig_delay + pi_time3
        rest_btwn_pi_bf_readsig_and_MWmix2 = MWmix_NV2_delay - pi_bf_read_sig_delay2 + pi_time3

        hilo_extra_delay           = when_pi_end + pi_to_hilo_extra_delay
        when_hilo_extra_end        = ion_RR_delay + 2e3
        if pi_pulse_bf_read == 'minus':
            when_hilo_extra_end    = pi_bf_read_sig_delay - 1e2
        ...
        noise_delay             = when_pi_ref_end2 + pi_to_hilo_extra_delay
        noise_duration          = pi_to_ion_delay - pi_to_hilo_extra_delay - 100

        ion_ref_RR_delay        = when_pi_ref_end2 + pi_to_ion_delay
        ...
        pi_bf_read_ref_delay    = ion_ref_RR_delay - 2*pi_time3 - shift_btwn_2NV_MW
        pi_bf_read_ref_delay2   = pi_bf_read_ref_delay + shift_btwn_2NV_MW
        ...
        when_hilo_extra_ref_end = ion_ref_RR_delay + 2e3
        if pi_pulse_bf_read == 'minus':
            when_hilo_extra_ref_end = pi_bf_read_ref_delay - 1e2

        MW_delay_for_AWG        = pi_delay - AWG_output_delay
        pi_to_MWmix_wait        = MWmix_delay - pi_delay - pi_time
        sig_to_ref_wait         = pi_ref_delay - MWmix_delay - MWmix_duration
        noise_to_MWmix_wait     = MWmix_ref_delay - noise_delay - noise_duration
        noise_to_MWmix_wait2    = MWmix_ref_NV2_delay - noise_delay - noise_duration
        
        MW_delay_for_AWG2        = pi_delay2 - AWG_output_delay
        pi_to_MWmix_wait2        = MWmix_NV2_delay - pi_delay2 - pi_time
        sig_to_ref_wait2         = pi_ref_delay2 - MWmix_NV2_delay - MWmix_duration

        m1=int(dutycycle_noise*int(noise_duration/4))
        rnd_arr = random_array(int(noise_duration/4),m1=m1)

        if self.loopCounter==0: sleepTime = 15
        else: sleepTime = 6
        if tau_ns>=30e6: sleepTime = 20
        if self.ifAWG:
            global ch1plot; global ch2plot
            ch1plot, ch2plot = AWG.send_T1SCCRRnoiseAWG_seq(int(MWmix_duration), int(AWG_buffer),
                            int(sig_to_ref_wait), pitime=int(pi_time), pi_to_MWmix_wait=int(pi_to_MWmix_wait),
                            pi_to_noise_wait=int(pi_to_hilo_extra_delay+shift_btwn_2NV_MW), noise_duration=int(noise_duration),
                            noise_to_MWmix_wait=int(noise_to_MWmix_wait),
                            rnd_arr=rnd_arr,amp_noise=amp_noise,
                            sleepTime=1,amp_MW_mix=amp_MW_mix)
            global ch1plot2; global ch2plot2
            ch1plot2, ch2plot2 = AWG2.send_T1SCCRRnoiseAWG_seq(int(MWmix_duration), int(AWG_buffer),
                            int(sig_to_ref_wait2), pitime=int(pi_time), pi_to_MWmix_wait=int(pi_to_MWmix_wait2),
                            pi_to_noise_wait=int(pi_to_hilo_extra_delay), noise_duration=int(noise_duration),
                            noise_to_MWmix_wait=int(noise_to_MWmix_wait2),
                            rnd_arr=rnd_arr,amp_noise=amp_noise,
                            sleepTime=sleepTime,amp_MW_mix=amp_MW_mix2)
##########################################################################################################
##########################################################################################################
        ...
        if pi_pulse_bf_read == 'plus':
            ...
            pulse_sequence += [spc.Pulse('MWswitch4',  pi_bf_read_sig_delay2, duration=int(pi_time3))]
##############################################################################################################################
##############################################################################################################################
        if not laser_init_delay == 0:
        ...
        # Ionization
        if ifTek:
            pulse_sequence += [spc.Pulse('noise',      noise_delay,          duration=int(noise_duration))] 
        ...
        if pi_pulse_bf_read == 'plus':
            ...
            pulse_sequence += [spc.Pulse('MWswitch4',  pi_bf_read_ref_delay2, duration=int(pi_time3))]
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T1SCCRRIrberDualNVNoiseAWGObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
