...
from B00_codes.T2ESCCRRIrberDualNVSingleRead import *
import dropbox
...
###########################################################################################e#########################
reps = -1
for iii in range(int(30)):
    # T2ESCCRRIrberDualNVSingleRead
    ...
    ifRndPhaseNoise = 1; AGBW = 1500e3; AGfreq = 3.125e6; AGamp = 0.5 # beware of heating!!

    taus1 = np.linspace(20080,80,201); tausArray = taus1
    ##############################################################################################################
    if True:
        # NV1
    ...
    ion_duration                 = 4200;      ion_duration2             = 4200
    ion_to_read_delay            = 2.5e6;     DAQ_duration              = 3e6
    ...
    ######################################### Scan params ########################################
    threshold_scanVpz            = 6.4;         threshold_scanVpz2        = -1
    ...
    settings = {'tausArray': tausArray,
                ...
                'hiLoMWPwr_channel':      hiLoMWPwr_channel,       'laserRead2_channel':     laserRead2_channel,
    ...
    #################################
    start = time.time()
    T2ESCCRRIrberDualNVSingleReadObject = T2ESCCRRIrberDualNVSingleRead(settings=settings, ifPlotPulse=ifPlotPulse) # this is implemented as an Instrument
    T2ESCCRRIrberDualNVSingleReadObject.runScan()
    print('Total time = ' + str(time.time() - start) + ' s')
    
    dataFilename = T2ESCCRRIrberDualNVSingleReadObject.getDataFilename()
    T2ESCCRRIrberDualNVSingleReadObject.close()
