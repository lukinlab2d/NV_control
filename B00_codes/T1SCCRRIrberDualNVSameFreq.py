...
class T1SCCRRIrberDualNVSameFreq(Instrument):

    def __init__(self, name='T1SCCRRIrberDualNVSameFreqObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T1SCCRRIrberDualNVSameFreq')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T1SCCRRIrberDualNVSameFreqObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.T1SCCRRIrberDualNVSameFreqObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        spinInit_pi_to_RR_delay= self.settings['spinInit_pi_to_RR_delay']
        ...
        ifIznNV2later           = self.settings['ifIznNV2later']
        if sweepWhich == 'ti':
            ion_duration         = tau_ns
        ...
        sig_to_ref_wait_revised = MWmix_ref_delay - when_MWmix_NV2_end
        ...
        if self.ifAWG:
            ...
            ch1plot, ch2plot = AWG.send_SCCRRPhotonStatIrberSameFreq_seq(int(MWmix_duration_both), int(AWG_buffer),
                            ...
                            rest_btwn_pi_bf_readsig_and_MWmix=rest_btwn_pi_bf_readsig_and_MWmix)
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T1SCCRRIrberDualNVSameFreqObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
