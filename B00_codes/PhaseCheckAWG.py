...
class PhaseCheckAWG(Instrument):

    def __init__(self, name='PhaseCheckAWGObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        self.phisArray = self.settings['phisArray']

        ifRandomized = self.settings['ifRandomized']
        if ifRandomized: np.random.shuffle(self.phisArray)
    ...
    def runScan(self):
        ...
        # For each iteration, sweep phi (sig.sweep calls set_raw() method of Parameter sig)
        # and measure Parameter sig, ref (each(sig,ref)) by calling get_raw() method of sig, ref
        loop = Loop(
            sig.sweep(keys=self.phisArray),
            delay = 0,
            sleepTimeAfterFinishing=0).each(sig, ref, sigOverRef,
                                            qctask(sig.plotPulseSequences),
                                            ).then(qctask(sig.turn_on_at_end))

        data = loop.get_data_set(name='PhaseCheckAWG')
        data.add_metadata(self.settings)
        self.data = data
        
        plot = QtPlot(
            data.PhaseCheckAWGObject_sig, # this is implemented as a Parameter
            figsize = (1200, 600),
            interval = 1,
            name = 'sig'
            )
        plot.add(data.PhaseCheckAWGObject_ref, name='ref')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/PhaseCheckAWGObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        ...
        self.PhaseCheckAWGObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, phi):
        # Make pulses, program Pulse Blaster
        print("Loop " + str(self.loopCounter))
        
        # Pulse parameters
        num_loops               = self.settings['num_loops']; tau              = self.settings['tau']
        ...
        AWG_buffer = self.settings['AWG_buffer']; 
        MW_duration = int(2*int((AWG_buffer + 2*pi2time + pitime + 2*tau + 1)/2))
        ...
        I = round(32767*np.cos(phi))
        Q = round(32767*np.sin(phi))

        global ch1plot; global ch2plot
        ch1plot, ch2plot = AWG.send_PhaseCheck_seq(pi_2time=int(pi2time), pitime = int(pitime), 
                                                   tau = int(tau), buffer=int(AWG_buffer), sig_to_ref_wait=int(sig_to_ref_wait), 
        ...
        print('Set phi to ' + str(phi) + " rad")
        if not self.settings['ifPlotPulse']: self.loopCounter += 1
    
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            if np.mod(self.loopCounter,5) == 0 or self.loopCounter == len(self.phisArray)-1: # plot every 5 sequences and plot the last seq
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.phisArray)-1: # only save first and last pulse sequence
                self.PhaseCheckAWGObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
