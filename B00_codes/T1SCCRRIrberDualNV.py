...
class T1SCCRRIrberDualNV(Instrument):

    def __init__(self, name='T1SCCRRIrberDualNVObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
    ...
    def runScan(self):
        ...
        data = loop.get_data_set(name='T1SCCRRIrberDualNV')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/T1SCCRRIrberDualNVObject_sig_set.dat'
    
class Signal(Parameter):
    def __init__(self, settings=None, name='sig', measurementObject=None, **kwargs):
        super().__init__(name, **kwargs)
        self.settings = settings
        self.T1SCCRRIrberDualNVObject = measurementObject
        self.loopCounter = 0
    ...
    def set_raw(self, tau_ns):
        ...
        if self.ifAWG:
            ...
            ch1plot2, ch2plot2 = AWG2.send_SCCRRPhotonStatIrber_seq(int(MWmix_duration), int(AWG_buffer),
                            ...
                            pi_pulse_bf_read=pi_pulse_bf_read,
                            rest_btwn_pi_and_pi_bf_readsig=rest_btwn_pi_and_pi_bf_readsig,
                            rest_btwn_pi_bf_readsig_and_MWmix=rest_btwn_pi_bf_readsig_and_MWmix2)
##########################################################################################################
##########################################################################################################
    ...
    def plotPulseSequences(self):
        ...
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.tausArray)-1: # only save first and last pulse sequence
                self.T1SCCRRIrberDualNVObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
