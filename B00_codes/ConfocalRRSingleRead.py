...
class ConfocalRRSingleRead(Instrument):

    def __init__(self, name='ConfocalRRSingleReadObject', settings=None, ifPlotPulse=True, **kwargs) -> None:
        ...
        self.galvo = DAQAnalogOutputs("ConfocalRRSingleReadDAQ", galvo_card_name, galvo_ao_channels)
    ...
    def runScan(self):
        ...
        data = loopy.get_data_set(name='ConfocalRRSingleRead')
    ...
    def getDataFilename(self):
        return 'C:/Users/lukin2dmaterials/' + self.data.location + '/ConfocalRRSingleReadObject_sig_set.dat'
...
class Signal(Parameter):
    def __init__(self, name='sig', measurementObject=None, settings=None, **kwargs):
        ...
        self.ConfocalRRSingleReadObject = measurementObject
        ...
        min_read_signal_delay       = np.min((read_signal_delay,read_signal_delay2))
        ...
        pulse_sequence += [spc.Pulse('Counter',      min_read_signal_delay,       duration=int(read_signal_duration))] # times are in ns
    ...
    def plotPulseSequences(self):
        if self.settings['ifPlotPulse']:
            ...
            if self.loopCounter == 0 or self.loopCounter == len(self.xArray)-1: # only save first and last pulse sequence
                self.ConfocalRRSingleReadObject.savedPulseSequencePlots[self.loopCounter] = deepcopy(fig)
            self.loopCounter += 1
